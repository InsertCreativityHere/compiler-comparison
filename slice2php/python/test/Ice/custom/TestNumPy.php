<?php
// Copyright (c) ZeroC, Inc.

// slice2php version 3.8.0-alpha.0
// <auto-generated>Generated from Slice file 'TestNumPy.ice'.</auto-generated>

namespace Test
{
}

namespace Test\NumPy
{
    global $Test_NumPy__t_BoolSeq1;

    if(!isset($Test_NumPy__t_BoolSeq1))
    {
        global $IcePHP__t_bool;
        $Test_NumPy__t_BoolSeq1 = IcePHP_defineSequence('::Test::NumPy::BoolSeq1', $IcePHP__t_bool);
    }

    global $Test_NumPy__t_BoolSeq2;

    if(!isset($Test_NumPy__t_BoolSeq2))
    {
        global $IcePHP__t_bool;
        $Test_NumPy__t_BoolSeq2 = IcePHP_defineSequence('::Test::NumPy::BoolSeq2', $IcePHP__t_bool);
    }

    global $Test_NumPy__t_ByteSeq1;

    if(!isset($Test_NumPy__t_ByteSeq1))
    {
        global $IcePHP__t_byte;
        $Test_NumPy__t_ByteSeq1 = IcePHP_defineSequence('::Test::NumPy::ByteSeq1', $IcePHP__t_byte);
    }

    global $Test_NumPy__t_ByteSeq2;

    if(!isset($Test_NumPy__t_ByteSeq2))
    {
        global $IcePHP__t_byte;
        $Test_NumPy__t_ByteSeq2 = IcePHP_defineSequence('::Test::NumPy::ByteSeq2', $IcePHP__t_byte);
    }

    global $Test_NumPy__t_ShortSeq1;

    if(!isset($Test_NumPy__t_ShortSeq1))
    {
        global $IcePHP__t_short;
        $Test_NumPy__t_ShortSeq1 = IcePHP_defineSequence('::Test::NumPy::ShortSeq1', $IcePHP__t_short);
    }

    global $Test_NumPy__t_ShortSeq2;

    if(!isset($Test_NumPy__t_ShortSeq2))
    {
        global $IcePHP__t_short;
        $Test_NumPy__t_ShortSeq2 = IcePHP_defineSequence('::Test::NumPy::ShortSeq2', $IcePHP__t_short);
    }

    global $Test_NumPy__t_IntSeq1;

    if(!isset($Test_NumPy__t_IntSeq1))
    {
        global $IcePHP__t_int;
        $Test_NumPy__t_IntSeq1 = IcePHP_defineSequence('::Test::NumPy::IntSeq1', $IcePHP__t_int);
    }

    global $Test_NumPy__t_IntSeq2;

    if(!isset($Test_NumPy__t_IntSeq2))
    {
        global $IcePHP__t_int;
        $Test_NumPy__t_IntSeq2 = IcePHP_defineSequence('::Test::NumPy::IntSeq2', $IcePHP__t_int);
    }

    global $Test_NumPy__t_LongSeq1;

    if(!isset($Test_NumPy__t_LongSeq1))
    {
        global $IcePHP__t_long;
        $Test_NumPy__t_LongSeq1 = IcePHP_defineSequence('::Test::NumPy::LongSeq1', $IcePHP__t_long);
    }

    global $Test_NumPy__t_LongSeq2;

    if(!isset($Test_NumPy__t_LongSeq2))
    {
        global $IcePHP__t_long;
        $Test_NumPy__t_LongSeq2 = IcePHP_defineSequence('::Test::NumPy::LongSeq2', $IcePHP__t_long);
    }

    global $Test_NumPy__t_FloatSeq1;

    if(!isset($Test_NumPy__t_FloatSeq1))
    {
        global $IcePHP__t_float;
        $Test_NumPy__t_FloatSeq1 = IcePHP_defineSequence('::Test::NumPy::FloatSeq1', $IcePHP__t_float);
    }

    global $Test_NumPy__t_FloatSeq2;

    if(!isset($Test_NumPy__t_FloatSeq2))
    {
        global $IcePHP__t_float;
        $Test_NumPy__t_FloatSeq2 = IcePHP_defineSequence('::Test::NumPy::FloatSeq2', $IcePHP__t_float);
    }

    global $Test_NumPy__t_DoubleSeq1;

    if(!isset($Test_NumPy__t_DoubleSeq1))
    {
        global $IcePHP__t_double;
        $Test_NumPy__t_DoubleSeq1 = IcePHP_defineSequence('::Test::NumPy::DoubleSeq1', $IcePHP__t_double);
    }

    global $Test_NumPy__t_DoubleSeq2;

    if(!isset($Test_NumPy__t_DoubleSeq2))
    {
        global $IcePHP__t_double;
        $Test_NumPy__t_DoubleSeq2 = IcePHP_defineSequence('::Test::NumPy::DoubleSeq2', $IcePHP__t_double);
    }

    global $Test_NumPy__t_Complex128Seq;

    if(!isset($Test_NumPy__t_Complex128Seq))
    {
        global $IcePHP__t_byte;
        $Test_NumPy__t_Complex128Seq = IcePHP_defineSequence('::Test::NumPy::Complex128Seq', $IcePHP__t_byte);
    }

    global $Test_NumPy__t_D;
    $Test_NumPy__t_D = IcePHP_declareClass('::Test::NumPy::D');

    global $Test_NumPy__t_D;
    class D extends \Ice\Value
    {
        public function __construct($boolSeq=\Ice\None, $byteSeq=\Ice\None, $shortSeq=\Ice\None, $intSeq=\Ice\None, $longSeq=\Ice\None, $floatSeq=\Ice\None, $doubleSeq=\Ice\None)
        {
            $this->boolSeq = $boolSeq;
            $this->byteSeq = $byteSeq;
            $this->shortSeq = $shortSeq;
            $this->intSeq = $intSeq;
            $this->longSeq = $longSeq;
            $this->floatSeq = $floatSeq;
            $this->doubleSeq = $doubleSeq;
        }

        public function ice_id()
        {
            return '::Test::NumPy::D';
        }

        public static function ice_staticId()
        {
            return '::Test::NumPy::D';
        }

        public function __toString(): string
        {
            global $Test_NumPy__t_D;
            return IcePHP_stringify($this, $Test_NumPy__t_D);
        }

        public $boolSeq;
        public $byteSeq;
        public $shortSeq;
        public $intSeq;
        public $longSeq;
        public $floatSeq;
        public $doubleSeq;
    }

    global $Ice__t_Value;
    global $Test_NumPy__t_BoolSeq1;
    global $Test_NumPy__t_ByteSeq1;
    global $Test_NumPy__t_ShortSeq1;
    global $Test_NumPy__t_IntSeq1;
    global $Test_NumPy__t_LongSeq1;
    global $Test_NumPy__t_FloatSeq1;
    global $Test_NumPy__t_DoubleSeq1;
    $Test_NumPy__t_D = IcePHP_defineClass('::Test::NumPy::D', '\\Test\\NumPy\\D', -1, $Ice__t_Value, array(
        array('boolSeq', $Test_NumPy__t_BoolSeq1, true, 1),
        array('byteSeq', $Test_NumPy__t_ByteSeq1, true, 2),
        array('shortSeq', $Test_NumPy__t_ShortSeq1, true, 3),
        array('intSeq', $Test_NumPy__t_IntSeq1, true, 4),
        array('longSeq', $Test_NumPy__t_LongSeq1, true, 5),
        array('floatSeq', $Test_NumPy__t_FloatSeq1, true, 6),
        array('doubleSeq', $Test_NumPy__t_DoubleSeq1, true, 7)));

    global $Test_NumPy__t_E;
    $Test_NumPy__t_E = IcePHP_declareClass('::Test::NumPy::E');

    global $Test_NumPy__t_E;
    class E extends \Ice\Value
    {
        public function __construct($boolSeq=null, $byteSeq=null, $shortSeq=null, $intSeq=null, $longSeq=null, $floatSeq=null, $doubleSeq=null)
        {
            $this->boolSeq = $boolSeq;
            $this->byteSeq = $byteSeq;
            $this->shortSeq = $shortSeq;
            $this->intSeq = $intSeq;
            $this->longSeq = $longSeq;
            $this->floatSeq = $floatSeq;
            $this->doubleSeq = $doubleSeq;
        }

        public function ice_id()
        {
            return '::Test::NumPy::E';
        }

        public static function ice_staticId()
        {
            return '::Test::NumPy::E';
        }

        public function __toString(): string
        {
            global $Test_NumPy__t_E;
            return IcePHP_stringify($this, $Test_NumPy__t_E);
        }

        public $boolSeq;
        public $byteSeq;
        public $shortSeq;
        public $intSeq;
        public $longSeq;
        public $floatSeq;
        public $doubleSeq;
    }

    global $Ice__t_Value;
    global $Test_NumPy__t_BoolSeq1;
    global $Test_NumPy__t_ByteSeq1;
    global $Test_NumPy__t_ShortSeq1;
    global $Test_NumPy__t_IntSeq1;
    global $Test_NumPy__t_LongSeq1;
    global $Test_NumPy__t_FloatSeq1;
    global $Test_NumPy__t_DoubleSeq1;
    $Test_NumPy__t_E = IcePHP_defineClass('::Test::NumPy::E', '\\Test\\NumPy\\E', -1, $Ice__t_Value, array(
        array('boolSeq', $Test_NumPy__t_BoolSeq1, false, 0),
        array('byteSeq', $Test_NumPy__t_ByteSeq1, false, 0),
        array('shortSeq', $Test_NumPy__t_ShortSeq1, false, 0),
        array('intSeq', $Test_NumPy__t_IntSeq1, false, 0),
        array('longSeq', $Test_NumPy__t_LongSeq1, false, 0),
        array('floatSeq', $Test_NumPy__t_FloatSeq1, false, 0),
        array('doubleSeq', $Test_NumPy__t_DoubleSeq1, false, 0)));

    global $Test_NumPy__t_Custom;
    global $Test_NumPy__t_CustomPrx;
    $Test_NumPy__t_CustomPrx = IcePHP_declareProxy('::Test::NumPy::Custom');

    global $Test_NumPy__t_Custom;
    global $Test_NumPy__t_CustomPrx;

    class CustomPrxHelper
    {
        public static function createProxy($communicator, $proxyString)
        {
            return  $communicator->stringToProxy($proxyString, '::Test::NumPy::Custom');
        }

        public static function checkedCast($proxy, ...$args)
        {
            return $proxy->ice_checkedCast('::Test::NumPy::Custom', ...$args);
        }

        public static function uncheckedCast($proxy, $facet=null)
        {
            return $proxy->ice_uncheckedCast('::Test::NumPy::Custom', $facet);
        }

        public static function ice_staticId()
        {
            return '::Test::NumPy::Custom';
        }
    }

    global $Ice__t_ObjectPrx;
    $Test_NumPy__t_CustomPrx = IcePHP_defineProxy('::Test::NumPy::Custom', $Ice__t_ObjectPrx, null);

    global $Test_NumPy__t_BoolSeq1;
    global $Test_NumPy__t_BoolSeq2;
    global $Test_NumPy__t_ByteSeq1;
    global $Test_NumPy__t_ByteSeq2;
    global $Test_NumPy__t_ShortSeq1;
    global $Test_NumPy__t_ShortSeq2;
    global $Test_NumPy__t_IntSeq1;
    global $Test_NumPy__t_IntSeq2;
    global $Test_NumPy__t_LongSeq1;
    global $Test_NumPy__t_LongSeq2;
    global $Test_NumPy__t_FloatSeq1;
    global $Test_NumPy__t_FloatSeq2;
    global $Test_NumPy__t_DoubleSeq1;
    global $Test_NumPy__t_DoubleSeq2;
    global $Test_NumPy__t_Complex128Seq;
    global $Test_NumPy__t_D;
    IcePHP_defineOperation($Test_NumPy__t_CustomPrx, 'opBoolSeq', 'opBoolSeq', 0, -1, array(array($Test_NumPy__t_BoolSeq1)), array(array($Test_NumPy__t_BoolSeq2)), array($Test_NumPy__t_BoolSeq1), null);
    IcePHP_defineOperation($Test_NumPy__t_CustomPrx, 'opByteSeq', 'opByteSeq', 0, -1, array(array($Test_NumPy__t_ByteSeq1)), array(array($Test_NumPy__t_ByteSeq2)), array($Test_NumPy__t_ByteSeq1), null);
    IcePHP_defineOperation($Test_NumPy__t_CustomPrx, 'opShortSeq', 'opShortSeq', 0, -1, array(array($Test_NumPy__t_ShortSeq1)), array(array($Test_NumPy__t_ShortSeq2)), array($Test_NumPy__t_ShortSeq1), null);
    IcePHP_defineOperation($Test_NumPy__t_CustomPrx, 'opIntSeq', 'opIntSeq', 0, -1, array(array($Test_NumPy__t_IntSeq1)), array(array($Test_NumPy__t_IntSeq2)), array($Test_NumPy__t_IntSeq1), null);
    IcePHP_defineOperation($Test_NumPy__t_CustomPrx, 'opLongSeq', 'opLongSeq', 0, -1, array(array($Test_NumPy__t_LongSeq1)), array(array($Test_NumPy__t_LongSeq2)), array($Test_NumPy__t_LongSeq1), null);
    IcePHP_defineOperation($Test_NumPy__t_CustomPrx, 'opFloatSeq', 'opFloatSeq', 0, -1, array(array($Test_NumPy__t_FloatSeq1)), array(array($Test_NumPy__t_FloatSeq2)), array($Test_NumPy__t_FloatSeq1), null);
    IcePHP_defineOperation($Test_NumPy__t_CustomPrx, 'opDoubleSeq', 'opDoubleSeq', 0, -1, array(array($Test_NumPy__t_DoubleSeq1)), array(array($Test_NumPy__t_DoubleSeq2)), array($Test_NumPy__t_DoubleSeq1), null);
    IcePHP_defineOperation($Test_NumPy__t_CustomPrx, 'opComplex128Seq', 'opComplex128Seq', 0, -1, array(array($Test_NumPy__t_Complex128Seq)), null, array($Test_NumPy__t_Complex128Seq), null);
    IcePHP_defineOperation($Test_NumPy__t_CustomPrx, 'opBoolMatrix', 'opBoolMatrix', 0, -1, null, null, array($Test_NumPy__t_BoolSeq1), null);
    IcePHP_defineOperation($Test_NumPy__t_CustomPrx, 'opByteMatrix', 'opByteMatrix', 0, -1, null, null, array($Test_NumPy__t_ByteSeq1), null);
    IcePHP_defineOperation($Test_NumPy__t_CustomPrx, 'opShortMatrix', 'opShortMatrix', 0, -1, null, null, array($Test_NumPy__t_ShortSeq1), null);
    IcePHP_defineOperation($Test_NumPy__t_CustomPrx, 'opIntMatrix', 'opIntMatrix', 0, -1, null, null, array($Test_NumPy__t_IntSeq1), null);
    IcePHP_defineOperation($Test_NumPy__t_CustomPrx, 'opLongMatrix', 'opLongMatrix', 0, -1, null, null, array($Test_NumPy__t_LongSeq1), null);
    IcePHP_defineOperation($Test_NumPy__t_CustomPrx, 'opFloatMatrix', 'opFloatMatrix', 0, -1, null, null, array($Test_NumPy__t_FloatSeq1), null);
    IcePHP_defineOperation($Test_NumPy__t_CustomPrx, 'opDoubleMatrix', 'opDoubleMatrix', 0, -1, null, null, array($Test_NumPy__t_DoubleSeq1), null);
    IcePHP_defineOperation($Test_NumPy__t_CustomPrx, 'opBogusNumpyArrayType', 'opBogusNumpyArrayType', 0, -1, null, null, array($Test_NumPy__t_BoolSeq1), null);
    IcePHP_defineOperation($Test_NumPy__t_CustomPrx, 'opD', 'opD', 0, -1, array(array($Test_NumPy__t_D)), null, array($Test_NumPy__t_D), null);
    IcePHP_defineOperation($Test_NumPy__t_CustomPrx, 'shutdown', 'shutdown', 0, -1, null, null, null, null);
}

namespace Test
{
}
