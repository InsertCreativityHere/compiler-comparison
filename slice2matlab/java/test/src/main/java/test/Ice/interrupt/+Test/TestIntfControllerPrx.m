classdef TestIntfControllerPrx < Ice.ObjectPrx
    ...
    %     interrupt
    %     interruptAsync - An asynchronous interrupt.
    %     resumeAdapter
    ...
    methods
        ...
        function interrupt(obj, context)
            %INTERRUPT
            %
            %   Input Arguments
            %     context - The request context.
            ...
            obj.iceInvoke('interrupt', 0, true, [], false, Test.TestIntfControllerPrx.interrupt_ex_, context);
        end

        function future = interruptAsync(obj, context)
            %INTERRUPTASYNC
            %
            %   Input Arguments
            %     context - The request context.
            ...
            %       Ice.Future scalar
            %
            %   See also interrupt, Ice.Future.
            
            arguments
                obj (1, 1) Test.TestIntfControllerPrx
                context (1, 1) dictionary = dictionary
            end
            future = obj.iceInvokeAsync('interrupt', 0, true, [], 0, [], Test.TestIntfControllerPrx.interrupt_ex_, context);
        end
    end
