...
classdef (Hidden) MyStructMyEnumD
    %MYSTRUCTMYENUMD Marshaling and unmarshaling support code for dictionary<::Test::MyStruct, ::Test::MyEnum>.
    %
    ...
    methods (Static)
        function write(os, d)
            if isempty(d)
                ...
                entries = d.entries;
                for i = 1:sz
                    Test.MyStruct.ice_write(os, entries{i, 1});
        ...
        function r = read(is)
            sz = is.readSize();
            r = configureDictionary('Test.MyStruct', 'Test.MyEnum');
            for i = 1:sz
