classdef BackgroundControllerPrx < Ice.ObjectPrx
    ...
    %     initializeSocketStatusAsync - An asynchronous initializeSocketStatus.
    %     pauseCall
    ...
    methods
        ...
        function initializeSocketStatus(obj, status, context)
            %INITIALIZESOCKETSTATUS
            %
            %   Input Arguments
            %     status
            ...
            os_.writeInt(status);
            obj.iceEndWriteParams(os_);
            obj.iceInvoke('initializeSocketStatus', 0, false, os_, false, {}, context);
        end

        function future = initializeSocketStatusAsync(obj, status, context)
            ...
            %       Ice.Future scalar
            %
            %   See also initializeSocketStatus, Ice.Future.
            ...
            os_.writeInt(status);
            obj.iceEndWriteParams(os_);
            future = obj.iceInvokeAsync('initializeSocketStatus', 0, false, os_, 0, [], {}, context);
        end
