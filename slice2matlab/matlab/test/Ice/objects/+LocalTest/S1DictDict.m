...
classdef (Hidden) S1DictDict
    %S1DICTDICT Marshaling and unmarshaling support code for dictionary<int, ::LocalTest::S1Dict>.
    %
    ...
    methods (Static)
        function write(os, d)
            if isempty(d)
                ...
                for i = 1:sz
                    ...
                    LocalTest.S1Dict.write(os, entries{i, 2}{1, 1});
                end
            end
        end
        function r = read(is)
        ...
        function r = convert(d)
            ...
            for i = 1:d.numEntries
                k = keys(i);
                v = values{i};
                r{k} = LocalTest.S1Dict.convert(v);
            end
        end
    end
end
