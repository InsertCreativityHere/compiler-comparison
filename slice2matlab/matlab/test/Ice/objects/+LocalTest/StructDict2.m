
classdef (Hidden) StructDict2
    %STRUCTDICT2 Marshaling and unmarshaling support code for dictionary<::LocalTest::StructKey, ::LocalTest::S1>.
    %
    %   Generated from LocalTest.ice by slice2matlab version 3.8.0-alpha.0

    methods (Access = private)
        function obj = StructDict2()
        end
    end
    methods (Static)
        function write(os, d)
            if isempty(d)
                os.writeSize(0);
            else
                sz = d.numEntries;
                os.writeSize(sz);
                entries = d.entries;
                for i = 1:sz
                    LocalTest.StructKey.ice_write(os, entries{i, 1});
                    LocalTest.S1.ice_write(os, entries{i, 2});
                end
            end
        end
        function r = read(is)
            sz = is.readSize();
            r = configureDictionary('LocalTest.StructKey', 'LocalTest.S1');
            for i = 1:sz
                k = LocalTest.StructKey.ice_read(is);
                v = LocalTest.S1.ice_read(is);
                r(k) = v;
            end
        end
        function r = convert(d)
            r = d;
            keys = d.keys;
            values = d.values;
            for i = 1:d.numEntries
                k = keys(i);
                v = values(i);
                r(k) = v.ice_convert();
            end
        end
    end
end
