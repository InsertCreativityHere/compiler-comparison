...
classdef elsifPrx < BEGIN.breakPrx
    methods
        function result = for(obj, freeze, if, methods, varargin)
            ...
            is_ = obj.iceInvoke('for', 0, true, os_, true, BEGIN.elsifPrx.for_ex_, varargin{:});
            is_.startEncapsulation();
            result = BEGIN.END.ice_read(is_);
            is_.endEncapsulation();
        end
        function r_ = forAsync(obj, freeze, if, methods, varargin)
            os_ = obj.iceStartWriteParams([]);
            os_.writeValue(freeze);
            os_.writeProxy(if);
            os_.writeInt(methods);
            ...
            r_ = obj.iceInvokeAsync('for', 0, true, os_, 1, @unmarshal, BEGIN.elsifPrx.for_ex_, varargin{:});
        end
    end
    methods(Static)
