...
classdef InitialPrx < Ice.ObjectPrx
    methods
        ...
        function result = getAbstractClass(obj, varargin)
            % getAbstractClass
            %
            % Parameters:
            %   context (containers.Map) - Optional request context.
            %
            % Returns (Test.AbstractClass)
            
            is_ = obj.iceInvoke('getAbstractClass', 0, true, [], true, {}, varargin{:});
            is_.startEncapsulation();
            result_h_ = IceInternal.ValueHolder();
            is_.readValue(@(v) result_h_.set(v), 'Test.AbstractClass');
            is_.readPendingValues();
            is_.endEncapsulation();
            result = result_h_.value;
        end
        function r_ = getAbstractClassAsync(obj, varargin)
            % getAbstractClassAsync
            %
            % Parameters:
            %   context (containers.Map) - Optional request context.
            ...
            function varargout = unmarshal(is_)
                is_.startEncapsulation();
                result = IceInternal.ValueHolder();
                is_.readValue(@(v) result.set(v), 'Test.AbstractClass');
                is_.readPendingValues();
                is_.endEncapsulation();
                varargout{1} = result.value;
            end
            r_ = obj.iceInvokeAsync('getAbstractClass', 0, true, [], 1, @unmarshal, {}, varargin{:});
        end
