classdef WstringClassPrx < Ice.ObjectPrx
    ...
    %   Creation
    %     Syntax
    %       prx = Test1.WstringClassPrx(communicator, proxyString)
    %
    %     Input Arguments
    ...
    %     checkedCast - Contacts the remote server to check if the target object implements Slice interface ::Test1::WstringClass.
    ...
    methods
        ...
        function [returnValue, s2] = opStruct(obj, s1, context)
            ...
            returnValue = Test1.WstringStruct.ice_read(is_);
            is_.endEncapsulation();
        end
        ...
        function throwExcept(obj, reason, context)
            ...
            obj.iceEndWriteParams(os_);
            obj.iceInvoke('throwExcept', 0, true, os_, false, Test1.WstringClassPrx.throwExcept_ex_, context);
        end
        ...
        function future = throwExceptAsync(obj, reason, context)
            ...
            os_.writeString(reason);
            obj.iceEndWriteParams(os_);
            future = obj.iceInvokeAsync('throwExcept', 0, true, os_, 0, [], Test1.WstringClassPrx.throwExcept_ex_, context);
        end
    end
    ...
    methods (Static)
        ...
        function r = checkedCast(p, varargin)
            ...
            %
            %   Output Arguments
            %     r - A Test1.WstringClassPrx scalar if the target object implements Slice interface 
            %       ::Test1::WstringClass; otherwise, an empty array of Test1.WstringClassPrx.
            %
            arguments
        ...
        function r = uncheckedCast(p, varargin)
            %UNCHECKEDCAST Creates a Test1.WstringClassPrx from another proxy without any validation.
            %
            %   Input Arguments
            %     p - The source proxy.
            %       Ice.ObjectPrx scalar | empty array of Ice.ObjectPrx
            ...
            %
            %   Output Arguments
            %     r - A new Test1.WstringClassPrx scalar, or an empty array when p is an empty array.
            %
            arguments
                p Ice.ObjectPrx {mustBeScalarOrEmpty}
            end
