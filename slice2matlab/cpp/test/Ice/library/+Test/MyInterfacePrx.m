classdef MyInterfacePrx < Ice.ObjectPrx
    ...
    methods
        function op(obj, throwIt, context)
            %OP
            %
            %   Input Arguments
            %     throwIt
            ...
            obj.iceInvoke('op', 0, true, os_, false, Test.MyInterfacePrx.op_ex_, context);
        end

        function future = opAsync(obj, throwIt, context)
            %OPASYNC
            %
            %   Input Arguments
            %     throwIt
            ...
            os_.writeBool(throwIt);
            obj.iceEndWriteParams(os_);
            future = obj.iceInvokeAsync('op', 0, true, os_, 0, [], Test.MyInterfacePrx.op_ex_, context);
        end
    end
