classdef TestIntfPrx < Ice.ObjectPrx
    ...
    %     throwDerived
    %     throwDerivedAsync - An asynchronous throwDerived.
    %
    ...
    methods
        function returnValue = opDerived(obj, context)
            %OPDERIVED
            %
            %   Input Arguments
            ...
            is_ = obj.iceInvoke('opDerived', 0, true, [], true, {}, context);
            is_.startEncapsulation();
            returnValue_h_ = IceInternal.ValueHolder();
            is_.readValue(@(v) returnValue_h_.set(v), 'Test.Base');
        ...
        function future = opDerivedAsync(obj, context)
            ...
            function varargout = unmarshal(is_)
                ...
                is_.readValue(@(v) returnValue.set(v), 'Test.Base');
                is_.readPendingValues();
                is_.endEncapsulation();
                varargout{1} = returnValue.value;
            end
            future = obj.iceInvokeAsync('opDerived', 0, true, [], 1, @unmarshal, {}, context);
        end

        function throwDerived(obj, context)
            %THROWDERIVED
            %
            %   Input Arguments
            %     context - The request context.
            ...
            obj.iceInvoke('throwDerived', 0, true, [], false, Test.TestIntfPrx.throwDerived_ex_, context);
        end

        function future = throwDerivedAsync(obj, context)
            %THROWDERIVEDASYNC
            %
            %   Input Arguments
            %     context - The request context.
            ...
            %       Ice.Future scalar
            %
            %   See also throwDerived, Ice.Future.
            
            arguments
                obj (1, 1) Test.TestIntfPrx
                context (1, 1) dictionary = dictionary
            end
            future = obj.iceInvokeAsync('throwDerived', 0, true, [], 0, [], Test.TestIntfPrx.throwDerived_ex_, context);
        end
    end
