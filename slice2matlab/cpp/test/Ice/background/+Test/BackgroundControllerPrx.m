...
classdef BackgroundControllerPrx < Ice.ObjectPrx
    methods
        ...
        function initializeSocketOperation(obj, status, varargin)
            os_ = obj.iceStartWriteParams([]);
            os_.writeInt(status);
            obj.iceEndWriteParams(os_);
            obj.iceInvoke('initializeSocketOperation', 0, false, os_, false, {}, varargin{:});
        end
        function r_ = initializeSocketOperationAsync(obj, status, varargin)
            os_ = obj.iceStartWriteParams([]);
            os_.writeInt(status);
            obj.iceEndWriteParams(os_);
            r_ = obj.iceInvokeAsync('initializeSocketOperation', 0, false, os_, 0, [], {}, varargin{:});
        end
