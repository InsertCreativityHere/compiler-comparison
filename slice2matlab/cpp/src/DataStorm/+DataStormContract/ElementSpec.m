classdef (Sealed) ElementSpec
    %ELEMENTSPEC Represents detailed information about topic elements, which can be a key or a filter.
    %
    %   Creation
    %     Syntax
    %       obj = DataStormContract.ElementSpec()
    %       obj = DataStormContract.ElementSpec(elements, id, name, value, peerId, peerName)
    %
    ...
    properties
        % ELEMENTS A sequence of data readers and writers associated with the key or filter.
        %   DataStormContract.ElementData vector
        elements (1, :) = {}
    ...
    methods (Static)
        function r = ice_read(is)
            r = DataStormContract.ElementSpec();
