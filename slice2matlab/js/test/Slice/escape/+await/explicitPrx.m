...
classdef explicitPrx < await.typeofPrx & await.casePrx
    methods
        function result = in(obj, internal, new, null, override, varargin)
            ...
            is_ = obj.iceInvoke('in', 0, true, os_, true, await.explicitPrx.in_ex_, varargin{:});
            is_.startEncapsulation();
            result = await.var.ice_read(is_);
            is_.endEncapsulation();
        end
        function r_ = inAsync(obj, internal, new, null, override, varargin)
            os_ = obj.iceStartWriteParams([]);
            await.break.ice_write(os_, internal);
            os_.writeProxy(new);
            os_.writeValue(null);
            ...
            r_ = obj.iceInvokeAsync('in', 0, true, os_, 1, @unmarshal, await.explicitPrx.in_ex_, varargin{:});
        end
    end
    methods(Static)
