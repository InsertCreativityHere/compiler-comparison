...
classdef ThrowerPrx < Ice.ObjectPrx
    methods
        ...
        function throwDispatchException(obj, replyStatus, varargin)
            os_ = obj.iceStartWriteParams([]);
            os_.writeByte(replyStatus);
            obj.iceEndWriteParams(os_);
            obj.iceInvoke('throwDispatchException', 0, false, os_, false, {}, varargin{:});
        end
        function r_ = throwDispatchExceptionAsync(obj, replyStatus, varargin)
            os_ = obj.iceStartWriteParams([]);
            os_.writeByte(replyStatus);
            obj.iceEndWriteParams(os_);
            r_ = obj.iceInvokeAsync('throwDispatchException', 0, false, os_, 0, [], {}, varargin{:});
        end
