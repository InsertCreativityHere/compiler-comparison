classdef MyInterfacePrx < Ice.ObjectPrx
    ...
    %   Creation
    %     Syntax
    %       prx = Inner.MyInterfacePrx(communicator, proxyString)
    %
    %     Input Arguments
    ...
    %     checkedCast - Contacts the remote server to check if the target object implements Slice interface ::Inner::MyInterface.
    ...
    methods
        ...
        function [returnValue, c2] = opMyClass(obj, c1, context)
            ...
            %   Input Arguments
            %     c1
            %       Inner.MyClass scalar | empty array of Inner.MyClass
            %     context - The request context.
            %       unconfigured dictionary (default) | dictionary(string, string) scalar
            %
            %   Output Arguments
            ...
            arguments
                obj (1, 1) Inner.MyInterfacePrx
                c1 Inner.MyClass {mustBeScalarOrEmpty}
                context (1, 1) dictionary = dictionary
            end
            os_ = obj.iceStartWriteParams([]);
            os_.writeValue(c1);
            ...
            is_.readValue(@(v) returnValue_h_.set(v), 'Inner.MyClass');
        ...
        function future = opMyClassAsync(obj, c1, context)
            ...
            function varargout = unmarshal(is_)
                ...
                is_.readValue(@(v) returnValue.set(v), 'Inner.MyClass');
                is_.readPendingValues();
                is_.endEncapsulation();
    ...
    methods (Static)
        ...
        function r = checkedCast(p, varargin)
            ...
            %
            %   Output Arguments
            %     r - A Inner.MyInterfacePrx scalar if the target object implements Slice interface 
            %       ::Inner::MyInterface; otherwise, an empty array of Inner.MyInterfacePrx.
            %
            arguments
        ...
        function r = uncheckedCast(p, varargin)
            %UNCHECKEDCAST Creates a Inner.MyInterfacePrx from another proxy without any validation.
            %
            %   Input Arguments
            %     p - The source proxy.
            %       Ice.ObjectPrx scalar | empty array of Ice.ObjectPrx
            ...
            %
            %   Output Arguments
            %     r - A new Inner.MyInterfacePrx scalar, or an empty array when p is an empty array.
            %
            arguments
                p Ice.ObjectPrx {mustBeScalarOrEmpty}
            end
