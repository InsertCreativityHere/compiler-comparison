...
classdef (Hidden) IntFixedStructDict
    %INTFIXEDSTRUCTDICT Marshaling and unmarshaling support code for dictionary<int, ::Test::FixedStruct>.
    %
        ...
        function write(os, d)
            if isempty(d)
                ...
                for i = 1:sz
                    ...
                    Test.FixedStruct.ice_write(os, entries{i, 2});
                end
            end
        end
        function r = read(is)
            sz = is.readSize();
            r = configureDictionary('int32', 'Test.FixedStruct');
            for i = 1:sz
                k = is.readInt();
