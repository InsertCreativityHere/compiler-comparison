classdef IntfPrx < Ice.ObjectPrx
    ...
    %     upCast
    %     upCastAsync - An asynchronous upCast.
    %
    ...
    methods
        ...
        function upCast(obj, context)
            ...
            obj.iceInvoke('upCast', 0, false, [], false, {}, context);
        end

        function future = upCastAsync(obj, context)
            %UPCASTASYNC
            %
            %   Input Arguments
            %     context - The request context.
            ...
            %       Ice.Future scalar
            %
            %   See also upCast, Ice.Future.
            
            arguments
                obj (1, 1) Clash.IntfPrx
                context (1, 1) dictionary = dictionary
            end
            future = obj.iceInvokeAsync('upCast', 0, false, [], 0, [], {}, context);
        end
        ...
        function op(obj, context, current, response, ex, sent, cookie, sync, result, istr, ostr, proxy, context_)
            %OP
            %
            %   Input Arguments
            %     context
            ...
            obj.iceInvoke('op', 0, false, os_, false, {}, context_);
        end

        function future = opAsync(obj, context, current, response, ex, sent, cookie, sync, result, istr, ostr, proxy, context_)
            %OPASYNC
            %
            %   Input Arguments
            %     context
            ...
            os_.writeStringOpt(1, proxy);
            obj.iceEndWriteParams(os_);
            future = obj.iceInvokeAsync('op', 0, false, os_, 0, [], {}, context_);
        end

        function [context, current, response, ex, sent, cookie, sync, result, istr, ostr, proxy] = opOut(obj, context)
            %OPOUT
            %
            %   Input Arguments
            %     context - The request context.
        ...
        function future = opOutAsync(obj, context)
            ...
            future = obj.iceInvokeAsync('opOut', 0, true, [], 11, @unmarshal, {}, context);
        end
    end
