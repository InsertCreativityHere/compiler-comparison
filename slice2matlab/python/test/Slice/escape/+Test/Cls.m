classdef Cls < Ice.Value
    ...
    %     Syntax
    %       obj = Test.Cls()
    %       obj = Test.Cls(s, context, current, response, upCast, typeId, del, cookie, ex, result, istr, ostr, inS, in, proxy)
    %
    ...
    properties
        % S
        %   Test.IntfPrx scalar | empty array of Test.IntfPrx
        s Test.IntfPrx {mustBeScalarOrEmpty} = Test.IntfPrx.empty
    ...
    methods (Access = protected)
        ...
        function iceReadImpl(obj, is)
            is.startSlice();
            obj.s = Test.IntfPrx.ice_read(is);
