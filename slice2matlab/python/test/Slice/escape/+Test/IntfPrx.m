classdef IntfPrx < Ice.ObjectPrx
    ...
    %   Creation
    %     Syntax
    %       prx = Test.IntfPrx(communicator, proxyString)
    %
    %     Input Arguments
    ...
    %     opField
    %     opFieldAsync - An asynchronous opField.
    %     opOut
    ...
    %     checkedCast - Contacts the remote server to check if the target object implements Slice interface ::Test::Intf.
    ...
    methods
        ...
        function opField(obj, name, value, context)
            %OPFIELD
            %
            %   Input Arguments
            %     name
            %       Test.field scalar | empty array of Test.field
            %     value
            %       Test.field scalar | empty array of Test.field
            %     context - The request context.
            %       unconfigured dictionary (default) | dictionary(string, string) scalar
            ...
            arguments
                obj (1, 1) Test.IntfPrx
                name Test.field {mustBeScalarOrEmpty}
                value Test.field {mustBeScalarOrEmpty}
                context (1, 1) dictionary = dictionary
            end
            os_ = obj.iceStartWriteParams([]);
            os_.writeValue(name);
            os_.writeValue(value);
            os_.writePendingValues();
            obj.iceEndWriteParams(os_);
            obj.iceInvoke('opField', 0, false, os_, false, {}, context);
        end

        function future = opFieldAsync(obj, name, value, context)
            %OPFIELDASYNC
            %
            %   Input Arguments
            %     name
            ...
            os_.writePendingValues();
            obj.iceEndWriteParams(os_);
            future = obj.iceInvokeAsync('opField', 0, false, os_, 0, [], {}, context);
        end
    end
    ...
    methods (Static)
        ...
        function r = checkedCast(p, varargin)
            ...
            %
            %   Output Arguments
            %     r - A Test.IntfPrx scalar if the target object implements Slice interface 
            %       ::Test::Intf; otherwise, an empty array of Test.IntfPrx.
            %
            arguments
            ...
            r = Ice.ObjectPrx.iceCheckedCast(p, Test.IntfPrx.ice_staticId(), 'Test.IntfPrx', varargin{:});
        end

        function r = uncheckedCast(p, varargin)
            %UNCHECKEDCAST Creates a Test.IntfPrx from another proxy without any validation.
            %
            %   Input Arguments
            %     p - The source proxy.
            %       Ice.ObjectPrx scalar | empty array of Ice.ObjectPrx
            ...
            %
            %   Output Arguments
            %     r - A new Test.IntfPrx scalar, or an empty array when p is an empty array.
            %
            arguments
                p Ice.ObjectPrx {mustBeScalarOrEmpty}
            end
            ...
            r = Ice.ObjectPrx.iceUncheckedCast(p, 'Test.IntfPrx', varargin{:});
        end
    end
end
