...
classdef ThrowerPrx < Ice.ObjectPrx
    methods
        ...
        function [result, p] = throwMarshalException(obj, varargin)
            is_ = obj.iceInvoke('throwMarshalException', 0, true, [], true, {}, varargin{:});
            is_.startEncapsulation();
            p = is_.readInt();
            result = is_.readInt();
            is_.endEncapsulation();
        end
        function r_ = throwMarshalExceptionAsync(obj, varargin)
            function varargout = unmarshal(is_)
                is_.startEncapsulation();
                p = is_.readInt();
                result = is_.readInt();
