...
% Generated from FileInfo.ice by slice2matlab version 3.7.9
...
classdef FileInfo
    ...
    methods
        function obj = FileInfo(path, checksum, size, executable)
            if nargin == 0
                obj.path = '';
                obj.checksum = [];
                obj.size = 0;
    ...
    methods(Static)
        function r = ice_read(is)
            r = IcePatch2.FileInfo(IceInternal.NoInit.Instance);
        ...
        function r = ice_readOpt(is, tag)
            if is.readOptional(tag, Ice.OptionalFormat.FSize)
                is.skip(4);
                r = IcePatch2.FileInfo.ice_read(is);
            else
                r = Ice.Unset;
            end
        end
        ...
        function ice_writeOpt(os, tag, v)
            if v ~= Ice.Unset && os.writeOptional(tag, Ice.OptionalFormat.FSize)
                pos = os.startSize();
                IcePatch2.FileInfo.ice_write(os, v);
                os.endSize(pos);
            end
        end
    end
