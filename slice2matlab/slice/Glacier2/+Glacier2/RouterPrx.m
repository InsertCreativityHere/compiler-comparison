...
classdef RouterPrx < Ice.RouterPrx
    methods
        function result = getCategoryForClient(obj, varargin)
            ...
            %
            % Returns (char) - The category. It's an empty string when `Glacier2.Server.Endpoints` is not configured.
        ...
        function result = createSession(obj, userId, password, varargin)
            % createSession   Creates a session for the client (caller) with the router. If a SessionManager is configured,
            % a proxy to a Session object is returned to the client. Otherwise, null is returned and only an
            ...
            %   context (containers.Map) - Optional request context.
            %
            % Returns (Glacier2.SessionPrx) - A proxy for the newly created session, or null if no SessionManager is configured.
            %
            % Exceptions:
        ...
        function r_ = createSessionAsync(obj, userId, password, varargin)
            % createSessionAsync   Creates a session for the client (caller) with the router. If a SessionManager is configured,
            % a proxy to a Session object is returned to the client. Otherwise, null is returned and only an
            % internal session (i.e., not visible to the client) is created.
            % If a non-null session proxy is returned, it must be configured to route through the router that created it.
            % This occurs automatically when the router is configured as the client's default router at the time the
        ...
        function result = createSessionFromSecureConnection(obj, varargin)
            ...
            % associated with the connection. If an SSLSessionManager is configured, a proxy to a Session
            ...
            %   context (containers.Map) - Optional request context.
            %
            % Returns (Glacier2.SessionPrx) - A proxy for the newly created session, or null if no SSLSessionManager is configured.
            %
            % Exceptions:
            %   Glacier2.CannotCreateSessionException - Thrown when the session cannot be created.
            %   Glacier2.PermissionDeniedException - Thrown when an authentication or authorization failure occurs.
