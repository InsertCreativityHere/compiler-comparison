...
classdef AdminPrx < Ice.ObjectPrx
    methods
        function addApplication(obj, descriptor, varargin)
            ...
            %
            % Exceptions:
            %   IceGrid.AccessDeniedException - Raised if the session doesn't
            %     hold the exclusive lock or if another session is holding the
            %     lock.
            %   IceGrid.DeploymentException - Raised if application deployment
            %     failed.
        ...
        function syncApplication(obj, descriptor, varargin)
            % syncApplication   Synchronize a deployed application with the given application
            ...
            %     hold the exclusive lock or if another session is holding the
            %     lock.
            %   IceGrid.ApplicationNotExistException - Raised if the application
            %     doesn't exist.
            %   IceGrid.DeploymentException - Raised if application deployment
            %     failed.
        ...
        function r_ = syncApplicationAsync(obj, descriptor, varargin)
            % syncApplicationAsync   Synchronize a deployed application with the given application
            % descriptor. This operation will replace the current descriptor
            % with this new descriptor.
            %
            % Parameters:
        ...
        function updateApplication(obj, descriptor, varargin)
            % updateApplication   Update a deployed application with the given update application
            % descriptor.
            %
            % Parameters:
        ...
        function r_ = updateApplicationAsync(obj, descriptor, varargin)
            % updateApplicationAsync   Update a deployed application with the given update application
            % descriptor.
            %
            % Parameters:
            %   descriptor (IceGrid.ApplicationUpdateDescriptor) - The update descriptor.
        ...
        function syncApplicationWithoutRestart(obj, descriptor, varargin)
            % syncApplicationWithoutRestart   Synchronize a deployed application with the given application
            % descriptor. This operation will replace the current descriptor
        ...
        function r_ = syncApplicationWithoutRestartAsync(obj, descriptor, varargin)
            % syncApplicationWithoutRestartAsync   Synchronize a deployed application with the given application
            % descriptor. This operation will replace the current descriptor
            % with this new descriptor only if no server restarts are
            % necessary for the update of the application. If some servers
            % need to be restarted, the synchronization is rejected with a
        ...
        function updateApplicationWithoutRestart(obj, descriptor, varargin)
            % updateApplicationWithoutRestart   Update a deployed application with the given update application
        ...
        function r_ = updateApplicationWithoutRestartAsync(obj, descriptor, varargin)
            % updateApplicationWithoutRestartAsync   Update a deployed application with the given update application
            % descriptor only if no server restarts are necessary for the
            % update of the application. If some servers need to be
            % restarted, the synchronization is rejected with a
            % DeploymentException.
        ...
        function instantiateServer(obj, application, node, desc, varargin)
            % instantiateServer   Instantiate a server template from an application on the given
            % node.
            %
            % Parameters:
            %   application (char) - The application name.
            %   node (char) - The name of the node where the server will be
            %     deployed.
            ...
            %   IceGrid.ApplicationNotExistException - Raised if the application
            %     doesn't exist.
            %   IceGrid.DeploymentException - Raised if server instantiation
            %     failed.
        ...
        function r_ = instantiateServerAsync(obj, application, node, desc, varargin)
            % instantiateServerAsync   Instantiate a server template from an application on the given
            % node.
            %
            % Parameters:
            %   application (char) - The application name.
        ...
        function patchApplication(obj, name, shutdown, varargin)
            ...
            % Parameters:
            %   name (char) - The application name.
            %   shutdown (logical) - If true, the servers depending on the data to
        ...
        function result = getDefaultApplicationDescriptor(obj, varargin)
            ...
            %
            % Exceptions:
            %   IceGrid.DeploymentException - Raised if the default application
        ...
        function result = getServerState(obj, id, varargin)
            ...
            %   IceGrid.NodeUnreachableException - Raised if the node could not be
            %     reached.
            %   IceGrid.ServerNotExistException - Raised if the server doesn't exist.
        ...
        function result = getServerPid(obj, id, varargin)
            % getServerPid   Get a server's system process id. The process id is operating
            % system dependent.
            %
            % Parameters:
            %   id (char) - The server id.
        ...
        function r_ = getServerPidAsync(obj, id, varargin)
            % getServerPidAsync   Get a server's system process id. The process id is operating
            % system dependent.
            %
            % Parameters:
            %   id (char) - The server id.
        ...
        function result = getServerAdminCategory(obj, varargin)
            % getServerAdminCategory   Get the category for server admin objects. You can manufacture a server admin
        ...
        function r_ = getServerAdminCategoryAsync(obj, varargin)
            % getServerAdminCategoryAsync   Get the category for server admin objects. You can manufacture a server admin
            % proxy from the admin proxy by changing its identity: use the server ID as name
            % and the returned category as category.
            %
            % Parameters:
        ...
        function enableServer(obj, id, enabled, varargin)
            % enableServer   Enable or disable a server. A disabled server can't be started
        ...
        function r_ = enableServerAsync(obj, id, enabled, varargin)
            % enableServerAsync   Enable or disable a server. A disabled server can't be started
            % on demand or administratively. The enable state of the server
            % is not persistent: if the node is shut down and restarted, the
            % server will be enabled by default.
            %
        ...
        function startServer(obj, id, varargin)
            ...
            %   IceGrid.ServerNotExistException - Raised if the server doesn't
            %     exist.
            %   IceGrid.ServerStartException - Raised if the server couldn't be
            %     started.
        ...
        function stopServer(obj, id, varargin)
            ...
            %   IceGrid.ServerNotExistException - Raised if the server doesn't
            %     exist.
            %   IceGrid.ServerStopException - Raised if the server couldn't be
            %     stopped.
        ...
        function patchServer(obj, id, shutdown, varargin)
            ...
            % Parameters:
            %   id (char) - The server id.
            %   shutdown (logical) - If true, servers depending on the data to patch
        ...
        function sendSignal(obj, id, signal, varargin)
            ...
            %
            % Exceptions:
            %   IceGrid.BadSignalException - Raised if the signal is not recognized
        ...
        function result = getAdapterInfo(obj, id, varargin)
            % getAdapterInfo   Get the adapter information for the replica group or adapter
            % with the given id.
            %
            % Parameters:
            ...
            %   context (containers.Map) - Optional request context.
            %
            % Returns (IceGrid.AdapterInfoSeq) - A sequence of adapter information structures. If the
            ...
            %
            % Exceptions:
            %   IceGrid.AdapterNotExistException - Raised if the adapter or
        ...
        function r_ = getAdapterInfoAsync(obj, id, varargin)
            % getAdapterInfoAsync   Get the adapter information for the replica group or adapter
            % with the given id.
            %
            % Parameters:
            %   id (char) - The adapter id.
        ...
        function removeAdapter(obj, id, varargin)
            ...
            %
            % Exceptions:
            %   IceGrid.AdapterNotExistException - Raised if the adapter doesn't
            %     exist.
            %   IceGrid.DeploymentException - Raised if application deployment failed.
        ...
        function addObject(obj_, obj, varargin)
            % addObject   Add an object to the object registry. IceGrid will get the
            ...
            %
            % Exceptions:
            %   IceGrid.DeploymentException - Raised if the object can't be
            %     added. This might be raised if the invocation on the proxy to
            %     get the object type failed.
            %   IceGrid.ObjectExistsException - Raised if the object is already
            %     registered.
        ...
        function r_ = addObjectAsync(obj_, obj, varargin)
            % addObjectAsync   Add an object to the object registry. IceGrid will get the
            % object type by calling ice_id on the given proxy. The object
            % must be reachable.
            %
            % Parameters:
        ...
        function updateObject(obj_, obj, varargin)
            % updateObject   Update an object in the object registry. Only objects added
            ...
            %   IceGrid.ObjectNotRegisteredException - Raised if the object isn't
        ...
        function r_ = updateObjectAsync(obj_, obj, varargin)
            % updateObjectAsync   Update an object in the object registry. Only objects added
            % with this interface can be updated with this operation. Objects
            % added with deployment descriptors should be updated with the
            % deployment mechanism.
            %
        ...
        function addObjectWithType(obj_, obj, type, varargin)
            % addObjectWithType   Add an object to the object registry and explicitly specify
            % its type.
            %
            % Parameters:
            %   obj (Ice.ObjectPrx) - The object to be added to the registry.
        ...
        function r_ = addObjectWithTypeAsync(obj_, obj, type, varargin)
            % addObjectWithTypeAsync   Add an object to the object registry and explicitly specify
            % its type.
            %
            % Parameters:
            %   obj (Ice.ObjectPrx) - The object to be added to the registry.
        ...
        function removeObject(obj, id, varargin)
            % removeObject   Remove an object from the object registry. Only objects added
            ...
            %
            % Parameters:
            %   id (Ice.Identity) - The identity of the object to be removed from the
            %     registry.
            %   context (containers.Map) - Optional request context.
            %
            % Exceptions:
        ...
        function r_ = removeObjectAsync(obj, id, varargin)
            % removeObjectAsync   Remove an object from the object registry. Only objects added
            % with this interface can be removed with this operation. Objects
            % added with deployment descriptors should be removed with the
            % deployment mechanism.
            %
        ...
        function result = getObjectInfosByType(obj, type, varargin)
            % getObjectInfosByType   Get the object info of all the registered objects with the
            % given type.
            %
            % Parameters:
        ...
        function r_ = getObjectInfosByTypeAsync(obj, type, varargin)
            % getObjectInfosByTypeAsync   Get the object info of all the registered objects with the
            % given type.
            %
            % Parameters:
            %   type (char) - The type of the object.
        ...
        function result = getAllObjectInfos(obj, expr, varargin)
            % getAllObjectInfos   Get the object info of all the registered objects whose stringified
            % identities match the given expression.
            %
            % Parameters:
            %   expr (char) - The expression to match against the stringified
            %     identities of registered objects. The expression may contain
            %     a trailing wildcard (*) character.
            %   context (containers.Map) - Optional request context.
            %
            % Returns (IceGrid.ObjectInfoSeq) - All the object infos with a stringified identity
        ...
        function r_ = getAllObjectInfosAsync(obj, expr, varargin)
            % getAllObjectInfosAsync   Get the object info of all the registered objects whose stringified
            % identities match the given expression.
            %
            % Parameters:
            %   expr (char) - The expression to match against the stringified
        ...
        function result = getNodeProcessorSocketCount(obj, name, varargin)
            % getNodeProcessorSocketCount   Get the number of physical processor sockets for the machine
            ...
            %   context (containers.Map) - Optional request context.
            %
            % Returns (int32) - The number of processor sockets or 1 if the number of
        ...
        function r_ = getNodeProcessorSocketCountAsync(obj, name, varargin)
            % getNodeProcessorSocketCountAsync   Get the number of physical processor sockets for the machine
            % running the node with the given name.
            %
            % Note that this method will return 1 on operating systems where
            % this can't be automatically determined and where the
        ...
        function result = getRegistryInfo(obj, name, varargin)
            ...
            % Exceptions:
            %   IceGrid.RegistryNotExistException - Raised if the registry doesn't exist.
            %   IceGrid.RegistryUnreachableException - Raised if the registry could not be
            %     reached.
    ...
    properties(Constant,Access=private)
        ...
        getServerState_ex_ = { 'IceGrid.DeploymentException', 'IceGrid.NodeUnreachableException', 'IceGrid.ServerNotExistException' }
        getServerPid_ex_ = { 'IceGrid.DeploymentException', 'IceGrid.NodeUnreachableException', 'IceGrid.ServerNotExistException' }
        getServerAdmin_ex_ = { 'IceGrid.DeploymentException', 'IceGrid.NodeUnreachableException', 'IceGrid.ServerNotExistException' }
        enableServer_ex_ = { 'IceGrid.DeploymentException', 'IceGrid.NodeUnreachableException', 'IceGrid.ServerNotExistException' }
        isServerEnabled_ex_ = { 'IceGrid.DeploymentException', 'IceGrid.NodeUnreachableException', 'IceGrid.ServerNotExistException' }
        startServer_ex_ = { 'IceGrid.DeploymentException', 'IceGrid.NodeUnreachableException', 'IceGrid.ServerNotExistException', 'IceGrid.ServerStartException' }
        stopServer_ex_ = { 'IceGrid.DeploymentException', 'IceGrid.NodeUnreachableException', 'IceGrid.ServerNotExistException', 'IceGrid.ServerStopException' }
        patchServer_ex_ = { 'IceGrid.DeploymentException', 'IceGrid.NodeUnreachableException', 'IceGrid.PatchException', 'IceGrid.ServerNotExistException' }
        sendSignal_ex_ = { 'IceGrid.BadSignalException', 'IceGrid.DeploymentException', 'IceGrid.NodeUnreachableException', 'IceGrid.ServerNotExistException' }
        ...
        addObject_ex_ = { 'IceGrid.DeploymentException', 'IceGrid.ObjectExistsException' }
        updateObject_ex_ = { 'IceGrid.DeploymentException', 'IceGrid.ObjectNotRegisteredException' }
        addObjectWithType_ex_ = { 'IceGrid.DeploymentException', 'IceGrid.ObjectExistsException' }
        removeObject_ex_ = { 'IceGrid.DeploymentException', 'IceGrid.ObjectNotRegisteredException' }
