...
if "Struct4" not in _M_Test.__dict__:
    ...
    class Struct4(object):
        def __init__(self, foo="foo", bar=""):
            self.foo = foo
            self.bar = bar

        def __hash__(self):
            _h = 0
            _h = 5 * _h + _builtins.hash(self.foo)
            _h = 5 * _h + _builtins.hash(self.bar)
        ...
        def __compare(self, other):
            if other is None:
                ...
                if self.foo is None or other.foo is None:
                    if self.foo != other.foo:
                        return (-1 if self.foo is None else 1)
                else:
                    if self.foo < other.foo:
                        return -1
                    elif self.foo > other.foo:
                        return 1
                if self.bar is None or other.bar is None:
                    if self.bar != other.bar:
                        return (-1 if self.bar is None else 1)
                else:
                    if self.bar < other.bar:
                        return -1
        ...
        def __repr__(self):
            return f"Test.Struct4({Ice.Util.format_fields(foo=self.foo, bar=self.bar)})"
