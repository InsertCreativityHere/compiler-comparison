...
if "Cls" not in _M_Clash.__dict__:
    _M_Clash.Cls = None
    class Cls(Ice.Value):
        def __init__(self, s=None, context="", current=0, response=0, upCast="", typeId=0, del=0, cookie=None, ex="", result=0, istr="", ostr="", inS="", proxy=""):
            self.s = s
        ...
        def __repr__(self):
            return f"Clash.Cls({Ice.Util.format_fields(s=self.s, context=self.context, current=self.current, response=self.response, upCast=self.upCast, typeId=self.typeId, del=self.del, cookie=self.cookie, ex=self.ex, result=self.result, istr=self.istr, ostr=self.ostr, inS=self.inS, proxy=self.proxy)})"
...
if "St" not in _M_Clash.__dict__:
    ...
    class St(object):
        ...
        def __repr__(self):
            return f"Clash.St({Ice.Util.format_fields(v=self.v, istr=self.istr, ostr=self.ostr, rhs=self.rhs)})"
