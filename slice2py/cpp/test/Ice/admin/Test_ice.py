# Copyright (c) ZeroC, Inc.

# slice2py version 3.8.0-alpha.0
# <auto-generated>Generated from Slice file 'Test.ice'.</auto-generated>

import Ice
import IcePy
import builtins as _builtins
import Ice.PropertyDict_ice

# Included module Ice
_M_Ice = Ice.openModule("Ice")

# Start of module Test
_M_Test = Ice.openModule("Test")
__name__ = "Test"

if "RemoteCommunicator" not in _M_Test.__dict__:
    _M_Test._t_RemoteCommunicatorPrx = IcePy.declareProxy("::Test::RemoteCommunicator")

if "RemoteCommunicatorPrx" not in _M_Test.__dict__:
    _M_Test.RemoteCommunicatorPrx = None
    class RemoteCommunicatorPrx(Ice.ObjectPrx):

        def __init__(self, communicator, proxyString):
            """
            Creates a new RemoteCommunicatorPrx proxy
            
            Parameters
            ----------
            communicator : Ice.Communicator
                The communicator of the new proxy.
            proxyString : str
                The string representation of the proxy.
            
            Raises
            ------
            ParseException
                Thrown when proxyString is not a valid proxy string.
            """
            super().__init__(communicator, proxyString)

        def getAdmin(self, context=None):
            return _M_Test.RemoteCommunicator._op_getAdmin.invoke(self, ((), context))

        def getAdminAsync(self, context=None):
            return _M_Test.RemoteCommunicator._op_getAdmin.invokeAsync(self, ((), context))

        def getChanges(self, context=None):
            return _M_Test.RemoteCommunicator._op_getChanges.invoke(self, ((), context))

        def getChangesAsync(self, context=None):
            return _M_Test.RemoteCommunicator._op_getChanges.invokeAsync(self, ((), context))

        def addUpdateCallback(self, context=None):
            return _M_Test.RemoteCommunicator._op_addUpdateCallback.invoke(self, ((), context))

        def addUpdateCallbackAsync(self, context=None):
            return _M_Test.RemoteCommunicator._op_addUpdateCallback.invokeAsync(self, ((), context))

        def removeUpdateCallback(self, context=None):
            return _M_Test.RemoteCommunicator._op_removeUpdateCallback.invoke(self, ((), context))

        def removeUpdateCallbackAsync(self, context=None):
            return _M_Test.RemoteCommunicator._op_removeUpdateCallback.invokeAsync(self, ((), context))

        def print(self, message, context=None):
            return _M_Test.RemoteCommunicator._op_print.invoke(self, ((message, ), context))

        def printAsync(self, message, context=None):
            return _M_Test.RemoteCommunicator._op_print.invokeAsync(self, ((message, ), context))

        def trace(self, category, message, context=None):
            return _M_Test.RemoteCommunicator._op_trace.invoke(self, ((category, message), context))

        def traceAsync(self, category, message, context=None):
            return _M_Test.RemoteCommunicator._op_trace.invokeAsync(self, ((category, message), context))

        def warning(self, message, context=None):
            return _M_Test.RemoteCommunicator._op_warning.invoke(self, ((message, ), context))

        def warningAsync(self, message, context=None):
            return _M_Test.RemoteCommunicator._op_warning.invokeAsync(self, ((message, ), context))

        def error(self, message, context=None):
            return _M_Test.RemoteCommunicator._op_error.invoke(self, ((message, ), context))

        def errorAsync(self, message, context=None):
            return _M_Test.RemoteCommunicator._op_error.invokeAsync(self, ((message, ), context))

        def shutdown(self, context=None):
            return _M_Test.RemoteCommunicator._op_shutdown.invoke(self, ((), context))

        def shutdownAsync(self, context=None):
            return _M_Test.RemoteCommunicator._op_shutdown.invokeAsync(self, ((), context))

        def waitForShutdown(self, context=None):
            return _M_Test.RemoteCommunicator._op_waitForShutdown.invoke(self, ((), context))

        def waitForShutdownAsync(self, context=None):
            return _M_Test.RemoteCommunicator._op_waitForShutdown.invokeAsync(self, ((), context))

        def destroy(self, context=None):
            return _M_Test.RemoteCommunicator._op_destroy.invoke(self, ((), context))

        def destroyAsync(self, context=None):
            return _M_Test.RemoteCommunicator._op_destroy.invokeAsync(self, ((), context))

        @staticmethod
        def checkedCast(proxy, facet=None, context=None):
            return Ice.checkedCast(_M_Test.RemoteCommunicatorPrx, proxy, facet, context)

        @staticmethod
        def checkedCastAsync(proxy, facet=None, context=None):
            return Ice.checkedCastAsync(_M_Test.RemoteCommunicatorPrx, proxy, facet, context)

        @staticmethod
        def uncheckedCast(proxy, facet=None):
            return Ice.uncheckedCast(_M_Test.RemoteCommunicatorPrx, proxy, facet)

        @staticmethod
        def ice_staticId():
            return "::Test::RemoteCommunicator"
    _M_Test._t_RemoteCommunicatorPrx = IcePy.defineProxy("::Test::RemoteCommunicator", RemoteCommunicatorPrx)

    _M_Test.RemoteCommunicatorPrx = RemoteCommunicatorPrx
    del RemoteCommunicatorPrx

    _M_Test.RemoteCommunicator = None
    class RemoteCommunicator(Ice.Object):

        def ice_ids(self, current):
            return ("::Ice::Object", "::Test::RemoteCommunicator")

        def ice_id(self, current):
            return "::Test::RemoteCommunicator"

        @staticmethod
        def ice_staticId():
            return "::Test::RemoteCommunicator"

        def getAdmin(self, current):
            raise NotImplementedError("servant method 'getAdmin' not implemented")

        def getChanges(self, current):
            raise NotImplementedError("servant method 'getChanges' not implemented")

        def addUpdateCallback(self, current):
            raise NotImplementedError("servant method 'addUpdateCallback' not implemented")

        def removeUpdateCallback(self, current):
            raise NotImplementedError("servant method 'removeUpdateCallback' not implemented")

        def print(self, message, current):
            raise NotImplementedError("servant method 'print' not implemented")

        def trace(self, category, message, current):
            raise NotImplementedError("servant method 'trace' not implemented")

        def warning(self, message, current):
            raise NotImplementedError("servant method 'warning' not implemented")

        def error(self, message, current):
            raise NotImplementedError("servant method 'error' not implemented")

        def shutdown(self, current):
            raise NotImplementedError("servant method 'shutdown' not implemented")

        def waitForShutdown(self, current):
            raise NotImplementedError("servant method 'waitForShutdown' not implemented")

        def destroy(self, current):
            raise NotImplementedError("servant method 'destroy' not implemented")

    RemoteCommunicator._op_getAdmin = IcePy.Operation("getAdmin", "getAdmin", Ice.OperationMode.Normal, None, (), (), (), ((), IcePy._t_ObjectPrx, False, 0), ())
    RemoteCommunicator._op_getChanges = IcePy.Operation("getChanges", "getChanges", Ice.OperationMode.Normal, None, (), (), (), ((), _M_Ice._t_PropertyDict, False, 0), ())
    RemoteCommunicator._op_addUpdateCallback = IcePy.Operation("addUpdateCallback", "addUpdateCallback", Ice.OperationMode.Normal, None, (), (), (), None, ())
    RemoteCommunicator._op_removeUpdateCallback = IcePy.Operation("removeUpdateCallback", "removeUpdateCallback", Ice.OperationMode.Normal, None, (), (), (), None, ())
    RemoteCommunicator._op_print = IcePy.Operation("print", "print", Ice.OperationMode.Normal, None, (), (((), IcePy._t_string, False, 0),), (), None, ())
    RemoteCommunicator._op_trace = IcePy.Operation("trace", "trace", Ice.OperationMode.Normal, None, (), (((), IcePy._t_string, False, 0), ((), IcePy._t_string, False, 0)), (), None, ())
    RemoteCommunicator._op_warning = IcePy.Operation("warning", "warning", Ice.OperationMode.Normal, None, (), (((), IcePy._t_string, False, 0),), (), None, ())
    RemoteCommunicator._op_error = IcePy.Operation("error", "error", Ice.OperationMode.Normal, None, (), (((), IcePy._t_string, False, 0),), (), None, ())
    RemoteCommunicator._op_shutdown = IcePy.Operation("shutdown", "shutdown", Ice.OperationMode.Normal, None, (), (), (), None, ())
    RemoteCommunicator._op_waitForShutdown = IcePy.Operation("waitForShutdown", "waitForShutdown", Ice.OperationMode.Normal, None, (), (), (), None, ())
    RemoteCommunicator._op_destroy = IcePy.Operation("destroy", "destroy", Ice.OperationMode.Normal, None, (), (), (), None, ())

    _M_Test.RemoteCommunicator = RemoteCommunicator
    del RemoteCommunicator

if "RemoteCommunicatorFactory" not in _M_Test.__dict__:
    _M_Test._t_RemoteCommunicatorFactoryPrx = IcePy.declareProxy("::Test::RemoteCommunicatorFactory")

if "RemoteCommunicatorFactoryPrx" not in _M_Test.__dict__:
    _M_Test.RemoteCommunicatorFactoryPrx = None
    class RemoteCommunicatorFactoryPrx(Ice.ObjectPrx):

        def __init__(self, communicator, proxyString):
            """
            Creates a new RemoteCommunicatorFactoryPrx proxy
            
            Parameters
            ----------
            communicator : Ice.Communicator
                The communicator of the new proxy.
            proxyString : str
                The string representation of the proxy.
            
            Raises
            ------
            ParseException
                Thrown when proxyString is not a valid proxy string.
            """
            super().__init__(communicator, proxyString)

        def createCommunicator(self, props, context=None):
            return _M_Test.RemoteCommunicatorFactory._op_createCommunicator.invoke(self, ((props, ), context))

        def createCommunicatorAsync(self, props, context=None):
            return _M_Test.RemoteCommunicatorFactory._op_createCommunicator.invokeAsync(self, ((props, ), context))

        def shutdown(self, context=None):
            return _M_Test.RemoteCommunicatorFactory._op_shutdown.invoke(self, ((), context))

        def shutdownAsync(self, context=None):
            return _M_Test.RemoteCommunicatorFactory._op_shutdown.invokeAsync(self, ((), context))

        @staticmethod
        def checkedCast(proxy, facet=None, context=None):
            return Ice.checkedCast(_M_Test.RemoteCommunicatorFactoryPrx, proxy, facet, context)

        @staticmethod
        def checkedCastAsync(proxy, facet=None, context=None):
            return Ice.checkedCastAsync(_M_Test.RemoteCommunicatorFactoryPrx, proxy, facet, context)

        @staticmethod
        def uncheckedCast(proxy, facet=None):
            return Ice.uncheckedCast(_M_Test.RemoteCommunicatorFactoryPrx, proxy, facet)

        @staticmethod
        def ice_staticId():
            return "::Test::RemoteCommunicatorFactory"
    _M_Test._t_RemoteCommunicatorFactoryPrx = IcePy.defineProxy("::Test::RemoteCommunicatorFactory", RemoteCommunicatorFactoryPrx)

    _M_Test.RemoteCommunicatorFactoryPrx = RemoteCommunicatorFactoryPrx
    del RemoteCommunicatorFactoryPrx

    _M_Test.RemoteCommunicatorFactory = None
    class RemoteCommunicatorFactory(Ice.Object):

        def ice_ids(self, current):
            return ("::Ice::Object", "::Test::RemoteCommunicatorFactory")

        def ice_id(self, current):
            return "::Test::RemoteCommunicatorFactory"

        @staticmethod
        def ice_staticId():
            return "::Test::RemoteCommunicatorFactory"

        def createCommunicator(self, props, current):
            raise NotImplementedError("servant method 'createCommunicator' not implemented")

        def shutdown(self, current):
            raise NotImplementedError("servant method 'shutdown' not implemented")

    RemoteCommunicatorFactory._op_createCommunicator = IcePy.Operation("createCommunicator", "createCommunicator", Ice.OperationMode.Normal, None, (), (((), _M_Ice._t_PropertyDict, False, 0),), (), ((), _M_Test._t_RemoteCommunicatorPrx, False, 0), ())
    RemoteCommunicatorFactory._op_shutdown = IcePy.Operation("shutdown", "shutdown", Ice.OperationMode.Normal, None, (), (), (), None, ())

    _M_Test.RemoteCommunicatorFactory = RemoteCommunicatorFactory
    del RemoteCommunicatorFactory

if "TestFacet" not in _M_Test.__dict__:
    _M_Test._t_TestFacetPrx = IcePy.declareProxy("::Test::TestFacet")

if "TestFacetPrx" not in _M_Test.__dict__:
    _M_Test.TestFacetPrx = None
    class TestFacetPrx(Ice.ObjectPrx):

        def __init__(self, communicator, proxyString):
            """
            Creates a new TestFacetPrx proxy
            
            Parameters
            ----------
            communicator : Ice.Communicator
                The communicator of the new proxy.
            proxyString : str
                The string representation of the proxy.
            
            Raises
            ------
            ParseException
                Thrown when proxyString is not a valid proxy string.
            """
            super().__init__(communicator, proxyString)

        def op(self, context=None):
            return _M_Test.TestFacet._op_op.invoke(self, ((), context))

        def opAsync(self, context=None):
            return _M_Test.TestFacet._op_op.invokeAsync(self, ((), context))

        @staticmethod
        def checkedCast(proxy, facet=None, context=None):
            return Ice.checkedCast(_M_Test.TestFacetPrx, proxy, facet, context)

        @staticmethod
        def checkedCastAsync(proxy, facet=None, context=None):
            return Ice.checkedCastAsync(_M_Test.TestFacetPrx, proxy, facet, context)

        @staticmethod
        def uncheckedCast(proxy, facet=None):
            return Ice.uncheckedCast(_M_Test.TestFacetPrx, proxy, facet)

        @staticmethod
        def ice_staticId():
            return "::Test::TestFacet"
    _M_Test._t_TestFacetPrx = IcePy.defineProxy("::Test::TestFacet", TestFacetPrx)

    _M_Test.TestFacetPrx = TestFacetPrx
    del TestFacetPrx

    _M_Test.TestFacet = None
    class TestFacet(Ice.Object):

        def ice_ids(self, current):
            return ("::Ice::Object", "::Test::TestFacet")

        def ice_id(self, current):
            return "::Test::TestFacet"

        @staticmethod
        def ice_staticId():
            return "::Test::TestFacet"

        def op(self, current):
            raise NotImplementedError("servant method 'op' not implemented")

    TestFacet._op_op = IcePy.Operation("op", "op", Ice.OperationMode.Normal, None, (), (), (), None, ())

    _M_Test.TestFacet = TestFacet
    del TestFacet

# End of module Test
