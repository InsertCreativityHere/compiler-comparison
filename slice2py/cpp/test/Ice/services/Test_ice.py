# Copyright (c) ZeroC, Inc.

# slice2py version 3.8.0-alpha.0
# <auto-generated>Generated from Slice file 'Test.ice'.</auto-generated>

import Ice
import IcePy
import builtins as _builtins

# Start of module Test
_M_Test = Ice.openModule("Test")
__name__ = "Test"

if "Clock" not in _M_Test.__dict__:
    _M_Test._t_ClockPrx = IcePy.declareProxy("::Test::Clock")

if "ClockPrx" not in _M_Test.__dict__:
    _M_Test.ClockPrx = None
    class ClockPrx(Ice.ObjectPrx):

        def __init__(self, communicator, proxyString):
            """
            Creates a new ClockPrx proxy
            
            Parameters
            ----------
            communicator : Ice.Communicator
                The communicator of the new proxy.
            proxyString : str
                The string representation of the proxy.
            
            Raises
            ------
            ParseException
                Thrown when proxyString is not a valid proxy string.
            """
            super().__init__(communicator, proxyString)

        def tick(self, time, context=None):
            return _M_Test.Clock._op_tick.invoke(self, ((time, ), context))

        def tickAsync(self, time, context=None):
            return _M_Test.Clock._op_tick.invokeAsync(self, ((time, ), context))

        @staticmethod
        def checkedCast(proxy, facet=None, context=None):
            return Ice.checkedCast(_M_Test.ClockPrx, proxy, facet, context)

        @staticmethod
        def checkedCastAsync(proxy, facet=None, context=None):
            return Ice.checkedCastAsync(_M_Test.ClockPrx, proxy, facet, context)

        @staticmethod
        def uncheckedCast(proxy, facet=None):
            return Ice.uncheckedCast(_M_Test.ClockPrx, proxy, facet)

        @staticmethod
        def ice_staticId():
            return "::Test::Clock"
    _M_Test._t_ClockPrx = IcePy.defineProxy("::Test::Clock", ClockPrx)

    _M_Test.ClockPrx = ClockPrx
    del ClockPrx

    _M_Test.Clock = None
    class Clock(Ice.Object):

        def ice_ids(self, current):
            return ("::Ice::Object", "::Test::Clock")

        def ice_id(self, current):
            return "::Test::Clock"

        @staticmethod
        def ice_staticId():
            return "::Test::Clock"

        def tick(self, time, current):
            raise NotImplementedError("servant method 'tick' not implemented")

    Clock._op_tick = IcePy.Operation("tick", "tick", Ice.OperationMode.Normal, None, (), (((), IcePy._t_string, False, 0),), (), None, ())

    _M_Test.Clock = Clock
    del Clock

# End of module Test
