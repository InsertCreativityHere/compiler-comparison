...
class BackendPrx(ObjectPrx):
    ...
    def check(self, context: dict[str, str] | None = None) -> None:
        return Backend._op_check.invoke(self, ((), context))

    def checkAsync(self, context: dict[str, str] | None = None) -> Awaitable[None]:
        return Backend._op_check.invokeAsync(self, ((), context))
...
class Backend(Object, ABC):
    ...
    @abstractmethod
    def check(self, current: Current) -> None | Awaitable[None]:
        pass
...
Backend._op_check = IcePy.Operation(
    "check",
    "check",
