...
if "IntfPrx" not in _M_Clash.__dict__:
    ...
    class IntfPrx(Ice.ObjectPrx):
        ...
        def _delAsync(self, context=None):
    ...
    Intf._op_del = IcePy.Operation("del", "_del", Ice.OperationMode.Normal, None, ("python:identifier:_del",), (), (), None, ())
...
if "Cls" not in _M_Clash.__dict__:
    ...
    class Cls(Ice.Value):
        ...
        def __repr__(self):
            return f"Clash.Cls({Ice.Util.format_fields(s=self.s, context=self.context, current=self.current, response=self.response, upCast=self.upCast, typeId=self.typeId, _del=self._del, cookie=self.cookie, ex=self.ex, result=self.result, istr=self.istr, ostr=self.ostr, inS=self.inS, _in=self._in, proxy=self.proxy)})"
