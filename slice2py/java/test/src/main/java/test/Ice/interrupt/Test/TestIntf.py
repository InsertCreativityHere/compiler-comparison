...
from Test.InterruptedException import _Test_InterruptedException_t
...
class TestIntfPrx(ObjectPrx):
    ...
    def opIdempotent(self, context: dict[str, str] | None = None) -> None:
        return TestIntf._op_opIdempotent.invoke(self, ((), context))

    def opIdempotentAsync(self, context: dict[str, str] | None = None) -> Awaitable[None]:
        return TestIntf._op_opIdempotent.invokeAsync(self, ((), context))
...
TestIntf._op_opIdempotent = IcePy.Operation(
    "opIdempotent",
    "opIdempotent",
