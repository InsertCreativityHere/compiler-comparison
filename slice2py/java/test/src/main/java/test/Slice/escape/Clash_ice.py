...
if "Cls" not in _M_Clash.__dict__:
    _M_Clash.Cls = None
    class Cls(Ice.Value):
        def __init__(self, s=None, context="", current=0, response=0, typeId=0, del=0, cookie=None, ex="", result=0, istr="", ostr="", inS="", in="", proxy="", obj=0, getCookie=0):
            self.s = s
...
if "St" not in _M_Clash.__dict__:
    _M_Clash.St = None
    class St(object):
        def __init__(self, v="", istr=0, ostr=0, rhs=0, hashCode=""):
            self.v = v
            self.istr = istr
            self.ostr = ostr
            self.rhs = rhs
        ...
        def __repr__(self):
            return f"Clash.St({Ice.Util.format_fields(v=self.v, istr=self.istr, ostr=self.ostr, rhs=self.rhs, hashCode=self.hashCode)})"
