// Copyright (c) ZeroC, Inc.

// slice2java version 3.8.0-alpha.0
// <auto-generated>Generated from Slice file 'Test.ice'.</auto-generated>

package test.Ice.enums.Test;

@com.zeroc.Ice.SliceTypeId(value = "::Test::TestIntf")
public interface TestIntf extends com.zeroc.Ice.Object {
    /**
     * Holds the result of operation opByte.
     */
    public static class OpByteResult {
        public ByteEnum returnValue;

        public ByteEnum b2;

        /**
         * Default constructor.
         */
        public OpByteResult() {
        }

        public OpByteResult(ByteEnum returnValue, ByteEnum b2) {
            this.returnValue = returnValue;
            this.b2 = b2;
        }

        /**
         * Marshals the result for the opByte operation.
         * @param ostr the output stream to write this result to
         */
        public void write(com.zeroc.Ice.OutputStream ostr) {
            ByteEnum.ice_write(ostr, this.b2);
            ByteEnum.ice_write(ostr, this.returnValue);
        }

        /**
         * Unmarshals the result of the opByte operation.
         * @param istr the input stream to read this result from
         */
        public void read(com.zeroc.Ice.InputStream istr) {
            this.b2 = ByteEnum.ice_read(istr);
            this.returnValue = ByteEnum.ice_read(istr);
        }
    }

    /**
     * Holds the result of operation opShort.
     */
    public static class OpShortResult {
        public ShortEnum returnValue;

        public ShortEnum s2;

        /**
         * Default constructor.
         */
        public OpShortResult() {
        }

        public OpShortResult(ShortEnum returnValue, ShortEnum s2) {
            this.returnValue = returnValue;
            this.s2 = s2;
        }

        /**
         * Marshals the result for the opShort operation.
         * @param ostr the output stream to write this result to
         */
        public void write(com.zeroc.Ice.OutputStream ostr) {
            ShortEnum.ice_write(ostr, this.s2);
            ShortEnum.ice_write(ostr, this.returnValue);
        }

        /**
         * Unmarshals the result of the opShort operation.
         * @param istr the input stream to read this result from
         */
        public void read(com.zeroc.Ice.InputStream istr) {
            this.s2 = ShortEnum.ice_read(istr);
            this.returnValue = ShortEnum.ice_read(istr);
        }
    }

    /**
     * Holds the result of operation opInt.
     */
    public static class OpIntResult {
        public IntEnum returnValue;

        public IntEnum i2;

        /**
         * Default constructor.
         */
        public OpIntResult() {
        }

        public OpIntResult(IntEnum returnValue, IntEnum i2) {
            this.returnValue = returnValue;
            this.i2 = i2;
        }

        /**
         * Marshals the result for the opInt operation.
         * @param ostr the output stream to write this result to
         */
        public void write(com.zeroc.Ice.OutputStream ostr) {
            IntEnum.ice_write(ostr, this.i2);
            IntEnum.ice_write(ostr, this.returnValue);
        }

        /**
         * Unmarshals the result of the opInt operation.
         * @param istr the input stream to read this result from
         */
        public void read(com.zeroc.Ice.InputStream istr) {
            this.i2 = IntEnum.ice_read(istr);
            this.returnValue = IntEnum.ice_read(istr);
        }
    }

    /**
     * Holds the result of operation opSimple.
     */
    public static class OpSimpleResult {
        public SimpleEnum returnValue;

        public SimpleEnum s2;

        /**
         * Default constructor.
         */
        public OpSimpleResult() {
        }

        public OpSimpleResult(SimpleEnum returnValue, SimpleEnum s2) {
            this.returnValue = returnValue;
            this.s2 = s2;
        }

        /**
         * Marshals the result for the opSimple operation.
         * @param ostr the output stream to write this result to
         */
        public void write(com.zeroc.Ice.OutputStream ostr) {
            SimpleEnum.ice_write(ostr, this.s2);
            SimpleEnum.ice_write(ostr, this.returnValue);
        }

        /**
         * Unmarshals the result of the opSimple operation.
         * @param istr the input stream to read this result from
         */
        public void read(com.zeroc.Ice.InputStream istr) {
            this.s2 = SimpleEnum.ice_read(istr);
            this.returnValue = SimpleEnum.ice_read(istr);
        }
    }

    /**
     * Holds the result of operation opByteSeq.
     */
    public static class OpByteSeqResult {
        public ByteEnum[] returnValue;

        public ByteEnum[] b2;

        /**
         * Default constructor.
         */
        public OpByteSeqResult() {
        }

        public OpByteSeqResult(ByteEnum[] returnValue, ByteEnum[] b2) {
            this.returnValue = returnValue;
            this.b2 = b2;
        }

        /**
         * Marshals the result for the opByteSeq operation.
         * @param ostr the output stream to write this result to
         */
        public void write(com.zeroc.Ice.OutputStream ostr) {
            ByteEnumSeqHelper.write(ostr, this.b2);
            ByteEnumSeqHelper.write(ostr, this.returnValue);
        }

        /**
         * Unmarshals the result of the opByteSeq operation.
         * @param istr the input stream to read this result from
         */
        public void read(com.zeroc.Ice.InputStream istr) {
            this.b2 = ByteEnumSeqHelper.read(istr);
            this.returnValue = ByteEnumSeqHelper.read(istr);
        }
    }

    /**
     * Holds the result of operation opShortSeq.
     */
    public static class OpShortSeqResult {
        public ShortEnum[] returnValue;

        public ShortEnum[] s2;

        /**
         * Default constructor.
         */
        public OpShortSeqResult() {
        }

        public OpShortSeqResult(ShortEnum[] returnValue, ShortEnum[] s2) {
            this.returnValue = returnValue;
            this.s2 = s2;
        }

        /**
         * Marshals the result for the opShortSeq operation.
         * @param ostr the output stream to write this result to
         */
        public void write(com.zeroc.Ice.OutputStream ostr) {
            ShortEnumSeqHelper.write(ostr, this.s2);
            ShortEnumSeqHelper.write(ostr, this.returnValue);
        }

        /**
         * Unmarshals the result of the opShortSeq operation.
         * @param istr the input stream to read this result from
         */
        public void read(com.zeroc.Ice.InputStream istr) {
            this.s2 = ShortEnumSeqHelper.read(istr);
            this.returnValue = ShortEnumSeqHelper.read(istr);
        }
    }

    /**
     * Holds the result of operation opIntSeq.
     */
    public static class OpIntSeqResult {
        public IntEnum[] returnValue;

        public IntEnum[] i2;

        /**
         * Default constructor.
         */
        public OpIntSeqResult() {
        }

        public OpIntSeqResult(IntEnum[] returnValue, IntEnum[] i2) {
            this.returnValue = returnValue;
            this.i2 = i2;
        }

        /**
         * Marshals the result for the opIntSeq operation.
         * @param ostr the output stream to write this result to
         */
        public void write(com.zeroc.Ice.OutputStream ostr) {
            IntEnumSeqHelper.write(ostr, this.i2);
            IntEnumSeqHelper.write(ostr, this.returnValue);
        }

        /**
         * Unmarshals the result of the opIntSeq operation.
         * @param istr the input stream to read this result from
         */
        public void read(com.zeroc.Ice.InputStream istr) {
            this.i2 = IntEnumSeqHelper.read(istr);
            this.returnValue = IntEnumSeqHelper.read(istr);
        }
    }

    /**
     * Holds the result of operation opSimpleSeq.
     */
    public static class OpSimpleSeqResult {
        public SimpleEnum[] returnValue;

        public SimpleEnum[] s2;

        /**
         * Default constructor.
         */
        public OpSimpleSeqResult() {
        }

        public OpSimpleSeqResult(SimpleEnum[] returnValue, SimpleEnum[] s2) {
            this.returnValue = returnValue;
            this.s2 = s2;
        }

        /**
         * Marshals the result for the opSimpleSeq operation.
         * @param ostr the output stream to write this result to
         */
        public void write(com.zeroc.Ice.OutputStream ostr) {
            SimpleEnumSeqHelper.write(ostr, this.s2);
            SimpleEnumSeqHelper.write(ostr, this.returnValue);
        }

        /**
         * Unmarshals the result of the opSimpleSeq operation.
         * @param istr the input stream to read this result from
         */
        public void read(com.zeroc.Ice.InputStream istr) {
            this.s2 = SimpleEnumSeqHelper.read(istr);
            this.returnValue = SimpleEnumSeqHelper.read(istr);
        }
    }

    TestIntf.OpByteResult opByte(ByteEnum b1, com.zeroc.Ice.Current current);

    TestIntf.OpShortResult opShort(ShortEnum s1, com.zeroc.Ice.Current current);

    TestIntf.OpIntResult opInt(IntEnum i1, com.zeroc.Ice.Current current);

    TestIntf.OpSimpleResult opSimple(SimpleEnum s1, com.zeroc.Ice.Current current);

    TestIntf.OpByteSeqResult opByteSeq(ByteEnum[] b1, com.zeroc.Ice.Current current);

    TestIntf.OpShortSeqResult opShortSeq(ShortEnum[] s1, com.zeroc.Ice.Current current);

    TestIntf.OpIntSeqResult opIntSeq(IntEnum[] i1, com.zeroc.Ice.Current current);

    TestIntf.OpSimpleSeqResult opSimpleSeq(SimpleEnum[] s1, com.zeroc.Ice.Current current);

    void shutdown(com.zeroc.Ice.Current current);

    /**
     * Gets the type ID of the associated Slice interface.
     *
     * @return the string "::Test::TestIntf"
     */
    static java.lang.String ice_staticId() {
        return "::Test::TestIntf";
    }

    /**
     * Dispatches the operation opByte.
     *
     * @param obj the servant object
     * @param request the incoming request
     * @return a {@code CompletionStage} that will complete when the dispatch is done
     * @hidden
     */
    static java.util.concurrent.CompletionStage<com.zeroc.Ice.OutgoingResponse> _iceD_opByte(TestIntf obj, com.zeroc.Ice.IncomingRequest request) {
        request.current.checkNonIdempotent();
        com.zeroc.Ice.InputStream istr = request.inputStream;
        istr.startEncapsulation();
        ByteEnum iceP_b1;
        iceP_b1 = ByteEnum.ice_read(istr);
        istr.endEncapsulation();
        TestIntf.OpByteResult ret = obj.opByte(iceP_b1, request.current);
        var ostr = request.current.startReplyStream();
        ostr.startEncapsulation(request.current.encoding, null);
        ret.write(ostr);
        ostr.endEncapsulation();
        return java.util.concurrent.CompletableFuture.completedFuture(new com.zeroc.Ice.OutgoingResponse(ostr));
    }

    /**
     * Dispatches the operation opShort.
     *
     * @param obj the servant object
     * @param request the incoming request
     * @return a {@code CompletionStage} that will complete when the dispatch is done
     * @hidden
     */
    static java.util.concurrent.CompletionStage<com.zeroc.Ice.OutgoingResponse> _iceD_opShort(TestIntf obj, com.zeroc.Ice.IncomingRequest request) {
        request.current.checkNonIdempotent();
        com.zeroc.Ice.InputStream istr = request.inputStream;
        istr.startEncapsulation();
        ShortEnum iceP_s1;
        iceP_s1 = ShortEnum.ice_read(istr);
        istr.endEncapsulation();
        TestIntf.OpShortResult ret = obj.opShort(iceP_s1, request.current);
        var ostr = request.current.startReplyStream();
        ostr.startEncapsulation(request.current.encoding, null);
        ret.write(ostr);
        ostr.endEncapsulation();
        return java.util.concurrent.CompletableFuture.completedFuture(new com.zeroc.Ice.OutgoingResponse(ostr));
    }

    /**
     * Dispatches the operation opInt.
     *
     * @param obj the servant object
     * @param request the incoming request
     * @return a {@code CompletionStage} that will complete when the dispatch is done
     * @hidden
     */
    static java.util.concurrent.CompletionStage<com.zeroc.Ice.OutgoingResponse> _iceD_opInt(TestIntf obj, com.zeroc.Ice.IncomingRequest request) {
        request.current.checkNonIdempotent();
        com.zeroc.Ice.InputStream istr = request.inputStream;
        istr.startEncapsulation();
        IntEnum iceP_i1;
        iceP_i1 = IntEnum.ice_read(istr);
        istr.endEncapsulation();
        TestIntf.OpIntResult ret = obj.opInt(iceP_i1, request.current);
        var ostr = request.current.startReplyStream();
        ostr.startEncapsulation(request.current.encoding, null);
        ret.write(ostr);
        ostr.endEncapsulation();
        return java.util.concurrent.CompletableFuture.completedFuture(new com.zeroc.Ice.OutgoingResponse(ostr));
    }

    /**
     * Dispatches the operation opSimple.
     *
     * @param obj the servant object
     * @param request the incoming request
     * @return a {@code CompletionStage} that will complete when the dispatch is done
     * @hidden
     */
    static java.util.concurrent.CompletionStage<com.zeroc.Ice.OutgoingResponse> _iceD_opSimple(TestIntf obj, com.zeroc.Ice.IncomingRequest request) {
        request.current.checkNonIdempotent();
        com.zeroc.Ice.InputStream istr = request.inputStream;
        istr.startEncapsulation();
        SimpleEnum iceP_s1;
        iceP_s1 = SimpleEnum.ice_read(istr);
        istr.endEncapsulation();
        TestIntf.OpSimpleResult ret = obj.opSimple(iceP_s1, request.current);
        var ostr = request.current.startReplyStream();
        ostr.startEncapsulation(request.current.encoding, null);
        ret.write(ostr);
        ostr.endEncapsulation();
        return java.util.concurrent.CompletableFuture.completedFuture(new com.zeroc.Ice.OutgoingResponse(ostr));
    }

    /**
     * Dispatches the operation opByteSeq.
     *
     * @param obj the servant object
     * @param request the incoming request
     * @return a {@code CompletionStage} that will complete when the dispatch is done
     * @hidden
     */
    static java.util.concurrent.CompletionStage<com.zeroc.Ice.OutgoingResponse> _iceD_opByteSeq(TestIntf obj, com.zeroc.Ice.IncomingRequest request) {
        request.current.checkNonIdempotent();
        com.zeroc.Ice.InputStream istr = request.inputStream;
        istr.startEncapsulation();
        ByteEnum[] iceP_b1;
        iceP_b1 = ByteEnumSeqHelper.read(istr);
        istr.endEncapsulation();
        TestIntf.OpByteSeqResult ret = obj.opByteSeq(iceP_b1, request.current);
        var ostr = request.current.startReplyStream();
        ostr.startEncapsulation(request.current.encoding, null);
        ret.write(ostr);
        ostr.endEncapsulation();
        return java.util.concurrent.CompletableFuture.completedFuture(new com.zeroc.Ice.OutgoingResponse(ostr));
    }

    /**
     * Dispatches the operation opShortSeq.
     *
     * @param obj the servant object
     * @param request the incoming request
     * @return a {@code CompletionStage} that will complete when the dispatch is done
     * @hidden
     */
    static java.util.concurrent.CompletionStage<com.zeroc.Ice.OutgoingResponse> _iceD_opShortSeq(TestIntf obj, com.zeroc.Ice.IncomingRequest request) {
        request.current.checkNonIdempotent();
        com.zeroc.Ice.InputStream istr = request.inputStream;
        istr.startEncapsulation();
        ShortEnum[] iceP_s1;
        iceP_s1 = ShortEnumSeqHelper.read(istr);
        istr.endEncapsulation();
        TestIntf.OpShortSeqResult ret = obj.opShortSeq(iceP_s1, request.current);
        var ostr = request.current.startReplyStream();
        ostr.startEncapsulation(request.current.encoding, null);
        ret.write(ostr);
        ostr.endEncapsulation();
        return java.util.concurrent.CompletableFuture.completedFuture(new com.zeroc.Ice.OutgoingResponse(ostr));
    }

    /**
     * Dispatches the operation opIntSeq.
     *
     * @param obj the servant object
     * @param request the incoming request
     * @return a {@code CompletionStage} that will complete when the dispatch is done
     * @hidden
     */
    static java.util.concurrent.CompletionStage<com.zeroc.Ice.OutgoingResponse> _iceD_opIntSeq(TestIntf obj, com.zeroc.Ice.IncomingRequest request) {
        request.current.checkNonIdempotent();
        com.zeroc.Ice.InputStream istr = request.inputStream;
        istr.startEncapsulation();
        IntEnum[] iceP_i1;
        iceP_i1 = IntEnumSeqHelper.read(istr);
        istr.endEncapsulation();
        TestIntf.OpIntSeqResult ret = obj.opIntSeq(iceP_i1, request.current);
        var ostr = request.current.startReplyStream();
        ostr.startEncapsulation(request.current.encoding, null);
        ret.write(ostr);
        ostr.endEncapsulation();
        return java.util.concurrent.CompletableFuture.completedFuture(new com.zeroc.Ice.OutgoingResponse(ostr));
    }

    /**
     * Dispatches the operation opSimpleSeq.
     *
     * @param obj the servant object
     * @param request the incoming request
     * @return a {@code CompletionStage} that will complete when the dispatch is done
     * @hidden
     */
    static java.util.concurrent.CompletionStage<com.zeroc.Ice.OutgoingResponse> _iceD_opSimpleSeq(TestIntf obj, com.zeroc.Ice.IncomingRequest request) {
        request.current.checkNonIdempotent();
        com.zeroc.Ice.InputStream istr = request.inputStream;
        istr.startEncapsulation();
        SimpleEnum[] iceP_s1;
        iceP_s1 = SimpleEnumSeqHelper.read(istr);
        istr.endEncapsulation();
        TestIntf.OpSimpleSeqResult ret = obj.opSimpleSeq(iceP_s1, request.current);
        var ostr = request.current.startReplyStream();
        ostr.startEncapsulation(request.current.encoding, null);
        ret.write(ostr);
        ostr.endEncapsulation();
        return java.util.concurrent.CompletableFuture.completedFuture(new com.zeroc.Ice.OutgoingResponse(ostr));
    }

    /**
     * Dispatches the operation shutdown.
     *
     * @param obj the servant object
     * @param request the incoming request
     * @return a {@code CompletionStage} that will complete when the dispatch is done
     * @hidden
     */
    static java.util.concurrent.CompletionStage<com.zeroc.Ice.OutgoingResponse> _iceD_shutdown(TestIntf obj, com.zeroc.Ice.IncomingRequest request) {
        request.current.checkNonIdempotent();
        request.inputStream.skipEmptyEncapsulation();
        obj.shutdown(request.current);
        return java.util.concurrent.CompletableFuture.completedFuture(request.current.createEmptyOutgoingResponse());
    }

    /**
     * Dispatches an incoming request to one of the methods of this generated interface, based on the operation name carried by the request.
     *
     * @param request the incoming request
     * @return the outgoing response
     * @throws com.zeroc.Ice.UserException if a {@code UserException} is thrown, Ice will marshal it as the response payload.
     */
    @Override
    default java.util.concurrent.CompletionStage<com.zeroc.Ice.OutgoingResponse> dispatch(com.zeroc.Ice.IncomingRequest request)
        throws com.zeroc.Ice.UserException {
        return switch (request.current.operation) {
            case "opByte" -> TestIntf._iceD_opByte(this, request);
            case "opShort" -> TestIntf._iceD_opShort(this, request);
            case "opInt" -> TestIntf._iceD_opInt(this, request);
            case "opSimple" -> TestIntf._iceD_opSimple(this, request);
            case "opByteSeq" -> TestIntf._iceD_opByteSeq(this, request);
            case "opShortSeq" -> TestIntf._iceD_opShortSeq(this, request);
            case "opIntSeq" -> TestIntf._iceD_opIntSeq(this, request);
            case "opSimpleSeq" -> TestIntf._iceD_opSimpleSeq(this, request);
            case "shutdown" -> TestIntf._iceD_shutdown(this, request);
            case "ice_id" -> com.zeroc.Ice.Object._iceD_ice_id(this, request);
            case "ice_ids" -> com.zeroc.Ice.Object._iceD_ice_ids(this, request);
            case "ice_isA" -> com.zeroc.Ice.Object._iceD_ice_isA(this, request);
            case "ice_ping" -> com.zeroc.Ice.Object._iceD_ice_ping(this, request);
            default -> throw new com.zeroc.Ice.OperationNotExistException();
        };
    }
}
