// Copyright (c) ZeroC, Inc.
//
// Ice version 3.8.0-alpha.0
//
// <auto-generated>
//
// Generated from file 'Key.ice'
//
// Warning: do not edit this file.
//
// </auto-generated>

package test.Slice.escape._abstract;

public interface catchPrx extends com.zeroc.Ice.ObjectPrx
{
    default int _checkedCast(int _clone)
    {
        return _checkedCast(_clone, com.zeroc.Ice.ObjectPrx.noExplicitContext);
    }

    default int _checkedCast(int _clone, java.util.Map<String, String> context)
    {
        return _iceI_checkedCastAsync(_clone, context, true).waitForResponse();
    }

    default java.util.concurrent.CompletableFuture<java.lang.Integer> checkedCastAsync(int _clone)
    {
        return _iceI_checkedCastAsync(_clone, com.zeroc.Ice.ObjectPrx.noExplicitContext, false);
    }

    default java.util.concurrent.CompletableFuture<java.lang.Integer> checkedCastAsync(int _clone, java.util.Map<String, String> context)
    {
        return _iceI_checkedCastAsync(_clone, context, false);
    }

    /**
     * @hidden
     * @param iceP_clone -
     * @param context -
     * @param sync -
     * @return -
     **/
    default com.zeroc.Ice.OutgoingAsync<java.lang.Integer> _iceI_checkedCastAsync(int iceP_clone, java.util.Map<String, String> context, boolean sync)
    {
        com.zeroc.Ice.OutgoingAsync<java.lang.Integer> f = new com.zeroc.Ice.OutgoingAsync<>(this, "checkedCast", null, sync, null);
        f.invoke(true, context, null, ostr -> {
                     ostr.writeInt(iceP_clone);
                 }, istr -> {
                     int ret;
                     ret = istr.readInt();
                     return ret;
                 });
        return f;
    }

    /**
     * Creates a new proxy that implements {@link catchPrx}.
     * @param communicator The communicator of the new proxy.
     * @param proxyString The string representation of the proxy.
     * @return The new proxy.
     **/
    public static catchPrx createProxy(com.zeroc.Ice.Communicator communicator, String proxyString)
    {
        return new _catchPrxI(com.zeroc.Ice.ObjectPrx.createProxy(communicator, proxyString));
    }

    /**
     * Contacts the remote server to verify that the object implements this type.
     * Raises a local exception if a communication error occurs.
     * @param obj The untyped proxy.
     * @return A proxy for this type, or null if the object does not support this type.
     **/
    static catchPrx checkedCast(com.zeroc.Ice.ObjectPrx obj)
    {
        return checkedCast(obj, noExplicitContext);
    }

    /**
     * Contacts the remote server to verify that the object implements this type.
     * Raises a local exception if a communication error occurs.
     * @param obj The untyped proxy.
     * @param context The Context map to send with the invocation.
     * @return A proxy for this type, or null if the object does not support this type.
     **/
    static catchPrx checkedCast(com.zeroc.Ice.ObjectPrx obj, java.util.Map<String, String> context)
    {
        return (obj != null && obj.ice_isA(ice_staticId(), context)) ? new _catchPrxI(obj) : null;
    }

    /**
     * Contacts the remote server to verify that a facet of the object implements this type.
     * Raises a local exception if a communication error occurs.
     * @param obj The untyped proxy.
     * @param facet The name of the desired facet.
     * @return A proxy for this type, or null if the object does not support this type.
     **/
    static catchPrx checkedCast(com.zeroc.Ice.ObjectPrx obj, String facet)
    {
        return checkedCast(obj, facet, noExplicitContext);
    }

    /**
     * Contacts the remote server to verify that a facet of the object implements this type.
     * Raises a local exception if a communication error occurs.
     * @param obj The untyped proxy.
     * @param facet The name of the desired facet.
     * @param context The Context map to send with the invocation.
     * @return A proxy for this type, or null if the object does not support this type.
     **/
    static catchPrx checkedCast(com.zeroc.Ice.ObjectPrx obj, String facet, java.util.Map<String, String> context)
    {
        return (obj == null) ? null : checkedCast(obj.ice_facet(facet), context);
    }

    /**
     * Downcasts the given proxy to this type without contacting the remote server.
     * @param obj The untyped proxy.
     * @return A proxy for this type.
     **/
    static catchPrx uncheckedCast(com.zeroc.Ice.ObjectPrx obj)
    {
        return (obj == null) ? null : new _catchPrxI(obj);
    }

    /**
     * Downcasts the given proxy to this type without contacting the remote server.
     * @param obj The untyped proxy.
     * @param facet The name of the desired facet.
     * @return A proxy for this type.
     **/
    static catchPrx uncheckedCast(com.zeroc.Ice.ObjectPrx obj, String facet)
    {
        return (obj == null) ? null : new _catchPrxI(obj.ice_facet(facet));
    }

    @Override
    catchPrx ice_context(java.util.Map<String, String> newContext);

    @Override
    catchPrx ice_adapterId(String newAdapterId);

    @Override
    catchPrx ice_endpoints(com.zeroc.Ice.Endpoint[] newEndpoints);

    @Override
    catchPrx ice_locatorCacheTimeout(int newTimeout);

    @Override
    catchPrx ice_invocationTimeout(int newTimeout);

    @Override
    catchPrx ice_connectionCached(boolean newCache);

    @Override
    catchPrx ice_endpointSelection(com.zeroc.Ice.EndpointSelectionType newType);

    @Override
    catchPrx ice_secure(boolean b);

    @Override
    catchPrx ice_encodingVersion(com.zeroc.Ice.EncodingVersion e);

    @Override
    catchPrx ice_preferSecure(boolean b);

    @Override
    catchPrx ice_router(com.zeroc.Ice.RouterPrx router);

    @Override
    catchPrx ice_locator(com.zeroc.Ice.LocatorPrx locator);

    @Override
    catchPrx ice_collocationOptimized(boolean b);

    @Override
    catchPrx ice_twoway();

    @Override
    catchPrx ice_oneway();

    @Override
    catchPrx ice_batchOneway();

    @Override
    catchPrx ice_datagram();

    @Override
    catchPrx ice_batchDatagram();

    @Override
    catchPrx ice_compress(boolean co);

    @Override
    catchPrx ice_connectionId(String connectionId);

    @Override
    catchPrx ice_fixed(com.zeroc.Ice.Connection connection);

    static String ice_staticId()
    {
        return "::abstract::catch";
    }

    /**
     * @hidden
     **/
    @Override
    default catchPrx _newInstance(com.zeroc.Ice.Reference ref)
    {
        return new _catchPrxI(ref);
    }
}
