// Copyright (c) ZeroC, Inc.

// slice2java version 3.8.0-alpha.0
// <auto-generated>Generated from Slice file 'IceStorm.ice'.</auto-generated>

package com.zeroc.IceStorm;

/**
 * The exception that is thrown when attempting to create a link that already exists.
 */
@com.zeroc.Ice.SliceTypeId(value = "::IceStorm::LinkExists")
public class LinkExists extends com.zeroc.Ice.UserException {
    /**
     * The name of the linked topic.
     */
    public java.lang.String name;

    /**
     * Constructs a {@code LinkExists}.
     */
    public LinkExists() {
        this.name = "";
    }

    /**
     * Constructs a {@code LinkExists} with values for all its fields.
     *
     * @param name The name of the linked topic.
     */
    public LinkExists(java.lang.String name) {
        this.name = name;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public java.lang.String ice_id() {
        return "::IceStorm::LinkExists";
    }

    /**
     * {@inheritDoc}
     * @hidden
     */
    @Override
    protected void _writeImpl(com.zeroc.Ice.OutputStream ostr_) {
        ostr_.startSlice("::IceStorm::LinkExists", -1, true);
        ostr_.writeString(name);
        ostr_.endSlice();
    }

    /**
     * {@inheritDoc}
     * @hidden
     */
    @Override
    protected void _readImpl(com.zeroc.Ice.InputStream istr_) {
        istr_.startSlice();
        name = istr_.readString();
        istr_.endSlice();
    }

    private static final long serialVersionUID = 6086710468001916896L;
}
