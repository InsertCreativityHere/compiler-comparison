// Copyright (c) ZeroC, Inc.

// slice2java version 3.8.0-alpha.0
// <auto-generated>Generated from Slice file 'Exception.ice'.</auto-generated>

package com.zeroc.IceGrid;

/**
 * The exception that is thrown when the registry update lock cannot be acquired.
 */
@com.zeroc.Ice.SliceTypeId(value = "::IceGrid::AccessDeniedException")
public class AccessDeniedException extends com.zeroc.Ice.UserException {
    /**
     * The id of the user holding the lock (if any).
     */
    public java.lang.String lockUserId;

    /**
     * Constructs a {@code AccessDeniedException}.
     */
    public AccessDeniedException() {
        this.lockUserId = "";
    }

    /**
     * Constructs a {@code AccessDeniedException} with values for all its fields.
     *
     * @param lockUserId The id of the user holding the lock (if any).
     */
    public AccessDeniedException(java.lang.String lockUserId) {
        this.lockUserId = lockUserId;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public java.lang.String ice_id() {
        return "::IceGrid::AccessDeniedException";
    }

    /**
     * {@inheritDoc}
     * @hidden
     */
    @Override
    protected void _writeImpl(com.zeroc.Ice.OutputStream ostr_) {
        ostr_.startSlice("::IceGrid::AccessDeniedException", -1, true);
        ostr_.writeString(lockUserId);
        ostr_.endSlice();
    }

    /**
     * {@inheritDoc}
     * @hidden
     */
    @Override
    protected void _readImpl(com.zeroc.Ice.InputStream istr_) {
        istr_.startSlice();
        lockUserId = istr_.readString();
        istr_.endSlice();
    }

    private static final long serialVersionUID = -8682100266437672405L;
}
