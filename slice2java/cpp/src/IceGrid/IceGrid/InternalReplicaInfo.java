// Copyright (c) ZeroC, Inc.

// slice2java version 3.8.0-alpha.0
// <auto-generated>Generated from Slice file 'Internal.ice'.</auto-generated>

package IceGrid;

/**
 * Information about an IceGrid registry replica.
 */
@com.zeroc.Ice.SliceTypeId(value = "::IceGrid::InternalReplicaInfo")
public class InternalReplicaInfo extends com.zeroc.Ice.Value {
    /**
     * The name of the registry.
     */
    public java.lang.String name;

    /**
     * The network name of the host running this registry (as defined in uname()).
     */
    public java.lang.String hostname;

    /**
     * Constructs a {@code InternalReplicaInfo}.
     */
    public InternalReplicaInfo() {
        this.name = "";
        this.hostname = "";
    }

    /**
     * Constructs a {@code InternalReplicaInfo} with values for all its fields.
     *
     * @param name The name of the registry.
     * @param hostname The network name of the host running this registry (as defined in uname()).
     */
    public InternalReplicaInfo(java.lang.String name, java.lang.String hostname) {
        this.name = name;
        this.hostname = hostname;
    }

    public InternalReplicaInfo clone() {
        return (InternalReplicaInfo) super.clone();
    }

    /**
     * Gets the type ID of the associated Slice interface.
     *
     * @return the string "::IceGrid::InternalReplicaInfo"
     */
    public static java.lang.String ice_staticId() {
        return "::IceGrid::InternalReplicaInfo";
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public java.lang.String ice_id() {
        return ice_staticId();
    }

    /**
     * {@inheritDoc}
     * @hidden
     */
    @Override
    protected void _iceWriteImpl(com.zeroc.Ice.OutputStream ostr_) {
        ostr_.startSlice(ice_staticId(), -1, true);
        ostr_.writeString(name);
        ostr_.writeString(hostname);
        ostr_.endSlice();
    }

    /**
     * {@inheritDoc}
     * @hidden
     */
    @Override
    protected void _iceReadImpl(com.zeroc.Ice.InputStream istr_) {
        istr_.startSlice();
        name = istr_.readString();
        hostname = istr_.readString();
        istr_.endSlice();
    }

    private static final long serialVersionUID = 2929619726087626810L;
}
