...
Test::TestActivation::_iceD_activateServantLocator(::IceInternal::Incoming& incoming)
{
    ...
    try
    {
        this->activateServantLocatorAsync(iceP_activate, [incomingPtr] { incomingPtr->response(); }, [incomingPtr](std::exception_ptr ex) { incomingPtr->completed(ex); }, incomingPtr->current());
    }
    catch (...)
    {
        incomingPtr->failed(::std::current_exception());
