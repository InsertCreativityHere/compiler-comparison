...
/// \cond INTERNAL
bool
_cpp_and::_cpp_break::_iceD_case(::IceInternal::Incoming& incoming)
{
    ...
    auto responseCB = [incomingPtr](::std::int32_t iceP_try)
    {
    ...
    try
    {
        this->caseAsync(iceP_catch, responseCB, [incomingPtr](std::exception_ptr ex) { incomingPtr->completed(ex); }, incomingPtr->current());
    }
    catch (...)
    {
...
/// \cond INTERNAL
bool
_cpp_and::func::_iceD_public(::IceInternal::Incoming& incoming)
{
    _iceCheckMode(::Ice::OperationMode::Normal, incoming.current().mode);
    incoming.readEmptyParams();
    this->_cpp_public(incoming.current());
