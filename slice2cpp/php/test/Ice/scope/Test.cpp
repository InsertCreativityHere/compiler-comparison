...
std::function<void()>
Test::IPrx::opCSeqAsync(const CSeq& iceP_s1, std::function<void(::Test::CSeq, ::Test::CSeq)> response, std::function<void(std::exception_ptr)> exception, std::function<void(bool)> sent, const Ice::Context& context) const
{
    auto responseCb = [response = std::move(response)](std::tuple<CSeq, CSeq>&& result) mutable
    {
        std::apply(std::move(response), std::move(result));
    };
    return IceInternal::makeLambdaOutgoing<std::tuple<CSeq, CSeq>>(std::move(responseCb), std::move(exception), std::move(sent), this, &Test::IPrx::_iceI_opCSeq, iceP_s1, context);
}
