...
std::function<void()>
Test::TestIntfPrx::jsExceptionAsync(std::function<void()> response, std::function<void(std::exception_ptr)> exception, std::function<void(bool)> sent, const Ice::Context& context) const
{
    return IceInternal::makeLambdaOutgoing<void>(std::move(response), std::move(exception), std::move(sent), this, &Test::TestIntfPrx::_iceI_jsException, context);
}
...
{
    static constexpr std::string_view operationName = "jsException";
...
Test::TestIntf::dispatch(Ice::IncomingRequest& request, std::function<void(Ice::OutgoingResponse)> sendResponse)
{
    ...
    switch (r.first - allOperations.begin())
    {
        ...
        case 8:
        {
            _iceD_jsException(request, std::move(sendResponse));
            break;
        }
        case 9:
        {
    ...
    std::function<void(Ice::OutgoingResponse)> sendResponse) // NOLINT(performance-unnecessary-value-param)
{
    checkNonIdempotent(request.current());
    request.inputStream().skipEmptyEncapsulation();
    this->userException(request.current());
    sendResponse(Ice::makeEmptyOutgoingResponse(request.current()));
}

void
Test::TestIntf::_iceD_jsException(
    Ice::IncomingRequest& request,
    std::function<void(Ice::OutgoingResponse)> sendResponse) // NOLINT(performance-unnecessary-value-param)
{
    checkNonIdempotent(request.current());
    request.inputStream().skipEmptyEncapsulation();
    this->jsException(request.current());
    sendResponse(Ice::makeEmptyOutgoingResponse(request.current()));
}
    ...
    std::function<void(Ice::OutgoingResponse)> sendResponse) // NOLINT(performance-unnecessary-value-param)
{
    ...
    const std::string ret = this->impossibleException(iceP_shouldThrow, request.current());
    ...
    std::function<void(Ice::OutgoingResponse)> sendResponse) // NOLINT(performance-unnecessary-value-param)
{
    ...
    istr->readAll(iceP_shouldThrow);
    istr->endEncapsulation();
    const std::string ret = this->intfUserException(iceP_shouldThrow, request.current());
    sendResponse(Ice::makeOutgoingResponse([&](Ice::OutputStream* ostr)
        {
            ostr->writeAll(ret);
        },
