...
::Ice::AsyncResultPtr
IceProxy::Test::Initial::_iceI_begin_throwTest1Notify(const ::Ice::Context& context, const ::IceInternal::CallbackBasePtr& del, const ::Ice::LocalObjectPtr& cookie, bool sync)
{
    _checkTwowayOnly(iceC_Test_Initial_throwTest1Notify_name, sync);
    ::IceInternal::OutgoingAsyncPtr result = new ::IceInternal::CallbackOutgoing(this, iceC_Test_Initial_throwTest1Notify_name, del, cookie, sync);
    try
    {
        result->prepare(iceC_Test_Initial_throwTest1Notify_name, ::Ice::Normal, context);
        result->writeEmptyParams();
        result->invoke(iceC_Test_Initial_throwTest1Notify_name);
    }
    catch(const ::Ice::Exception& ex)
    {
        result->abort(ex);
...
void
IceProxy::Test::Initial::end_throwTest1Notify(const ::Ice::AsyncResultPtr& result)
{
    ::Ice::AsyncResult::_check(result, this, iceC_Test_Initial_throwTest1Notify_name);
    if(!result->_waitForResponse())
    {
        try
        {
