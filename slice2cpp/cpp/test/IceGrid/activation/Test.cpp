...
std::function<void()>
Test::TestIntfPrx::failAsync(std::function<void()> response, std::function<void(std::exception_ptr)> exception, std::function<void(bool)> sent, const Ice::Context& context) const
{
    return IceInternal::makeLambdaOutgoing<void>(std::move(response), std::move(exception), std::move(sent), this, &Test::TestIntfPrx::_iceI_fail, context);
}
...
{
    static constexpr std::array<std::string_view, 6> allOperations{"fail", "ice_id", "ice_ids", "ice_isA", "ice_ping", "shutdown"};
