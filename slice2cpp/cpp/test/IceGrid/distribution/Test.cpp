...
/// \cond INTERNAL
bool
Test::TestIntf::_iceD_getServerFile(::IceInternal::Incoming& incoming)
{
    ...
    ::std::string ret = this->getServerFile(::std::move(iceP_path), incoming.current());
...
/// \cond INTERNAL
bool
Test::TestIntf::_iceD_getApplicationFile(::IceInternal::Incoming& incoming)
{
    _iceCheckMode(::Ice::OperationMode::Normal, incoming.current().mode);
    auto istr = incoming.startReadParams();
    ::std::string iceP_path;
    istr->readAll(iceP_path);
    incoming.endReadParams();
    ::std::string ret = this->getApplicationFile(::std::move(iceP_path), incoming.current());
    auto ostr = incoming.startWriteParams();
    ostr->writeAll(ret);
    incoming.endWriteParams();
    return true;
...
Test::TestIntf::_iceDispatch(::IceInternal::Incoming& incoming)
{
    ...
    switch(r.first - allOperations)
    {
        case 0:
        {
            return _iceD_getApplicationFile(incoming);
        }
        case 1:
        {
            return _iceD_getServerFile(incoming);
        }
        case 2:
        {
