    ...
    std::function<void(Ice::OutgoingResponse)> sendResponse) // NOLINT(performance-unnecessary-value-param)
{
    _iceCheckMode(Ice::OperationMode::Normal, request.current().mode);
    request.inputStream().skipEmptyEncapsulation();
    this->addUpdateCallback(request.current());
    ...
    std::function<void(Ice::OutgoingResponse)> sendResponse) // NOLINT(performance-unnecessary-value-param)
{
    _iceCheckMode(Ice::OperationMode::Normal, request.current().mode);
    request.inputStream().skipEmptyEncapsulation();
    this->removeUpdateCallback(request.current());
    sendResponse(Ice::makeEmptyOutgoingResponse(request.current()));
}
/// \endcond
...
{
    static constexpr std::array<std::string_view, 15> allOperations{"addUpdateCallback", "destroy", "error", "getAdmin", "getChanges", "ice_id", "ice_ids", "ice_isA", "ice_ping", "print", "removeUpdateCallback", "shutdown", "trace", "waitForShutdown", "warning"};
    ...
    switch (r.first - allOperations.begin())
    {
        case 0:
        {
            _iceD_addUpdateCallback(request, std::move(sendResponse));
            break;
        }
        case 1:
        {
        ...
        case 10:
        {
            _iceD_removeUpdateCallback(request, std::move(sendResponse));
            break;
        }
        case 11:
        {
