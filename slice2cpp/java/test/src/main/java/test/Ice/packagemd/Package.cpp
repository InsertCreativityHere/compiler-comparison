// Copyright (c) ZeroC, Inc.

// slice2cpp version 3.8.0-alpha.0
// <auto-generated>Generated from Slice file 'Package.ice'.</auto-generated>

#define ICE_BUILDING_GENERATED_CODE

#include "Package.h"
#include <Ice/AsyncResponseHandler.h>
#include <Ice/DefaultSliceLoader.h>
#include <Ice/OutgoingAsync.h>
#include <algorithm>
#include <array>

#if defined(_MSC_VER)
#   pragma warning(disable : 4458) // declaration of ... hides class member
#   pragma warning(disable : 4996) // ... was declared deprecated
#elif defined(__clang__)
#   pragma clang diagnostic ignored "-Wshadow"
#   pragma clang diagnostic ignored "-Wdeprecated-declarations"
#elif defined(__GNUC__)
#   pragma GCC diagnostic ignored "-Wshadow"
#   pragma GCC diagnostic ignored "-Wdeprecated-declarations"
#endif

#ifndef ICE_DISABLE_VERSION
#   if ICE_INT_VERSION  != 30850
#       error Ice version mismatch: an exact match is required for beta generated code
#   endif
#endif

namespace
{
    const IceInternal::ClassInit<::Test2::C1> iceC_Test2_C1_init;
    const IceInternal::ClassInit<::Test2::C2> iceC_Test2_C2_init;
    const IceInternal::ExceptionInit<::Test2::E1> iceC_Test2_E1_init;
    const IceInternal::ExceptionInit<::Test2::E2> iceC_Test2_E2_init;
    const IceInternal::ClassInit<::Test3::C1> iceC_Test3_C1_init;
    const IceInternal::ClassInit<::Test3::C2> iceC_Test3_C2_init;
    const IceInternal::ExceptionInit<::Test3::E1> iceC_Test3_E1_init;
    const IceInternal::ExceptionInit<::Test3::E2> iceC_Test3_E2_init;
}

const char*
Test2::C1::ice_staticId() noexcept
{
    return "::Test2::C1";
}

const char*
Test2::C1::ice_id() const noexcept
{
    return ice_staticId();
}

void
Test2::C1::ice_printFields(std::ostream& os) const
{
    Ice::print(os << "i = ", this->i);
}

Ice::ValuePtr
Test2::C1::_iceCloneImpl() const
{
    return CloneEnabler<C1>::clone(*this);
}

void
Test2::C1::_iceWriteImpl(Ice::OutputStream* ostr) const
{
    ostr->startSlice(ice_staticId(), -1, true);
    ostr->writeAll(this->i);
    ostr->endSlice();
}

void
Test2::C1::_iceReadImpl(Ice::InputStream* istr)
{
    istr->startSlice();
    istr->readAll(this->i);
    istr->endSlice();
}

const char*
Test2::C2::ice_staticId() noexcept
{
    return "::Test2::C2";
}

const char*
Test2::C2::ice_id() const noexcept
{
    return ice_staticId();
}

void
Test2::C2::ice_printFields(std::ostream& os) const
{
    C1::ice_printFields(os);
    Ice::print(os << ", l = ", this->l);
}

Ice::ValuePtr
Test2::C2::_iceCloneImpl() const
{
    return CloneEnabler<C2>::clone(*this);
}

void
Test2::C2::_iceWriteImpl(Ice::OutputStream* ostr) const
{
    ostr->startSlice(ice_staticId(), -1, false);
    ostr->writeAll(this->l);
    ostr->endSlice();
    C1::_iceWriteImpl(ostr);
}

void
Test2::C2::_iceReadImpl(Ice::InputStream* istr)
{
    istr->startSlice();
    istr->readAll(this->l);
    istr->endSlice();
    C1::_iceReadImpl(istr);
}

void
Test2::E1::ice_printFields(std::ostream& os) const
{
    Ice::print(os << "i = ", this->i);
}

const char*
Test2::E1::ice_staticId() noexcept
{
    return "::Test2::E1";
}

const char*
Test2::E1::ice_id() const noexcept
{
    return ice_staticId();
}

void
Test2::E1::ice_throw() const
{
    throw *this;
}

void
Test2::E1::_writeImpl(Ice::OutputStream* ostr) const
{
    ostr->startSlice(ice_staticId(), -1, true);
    ostr->writeAll(this->i);
    ostr->endSlice();
}

void
Test2::E1::_readImpl(Ice::InputStream* istr)
{
    istr->startSlice();
    istr->readAll(this->i);
    istr->endSlice();
}

void
Test2::E2::ice_printFields(std::ostream& os) const
{
    E1::ice_printFields(os);
    Ice::print(os << ", l = ", this->l);
}

const char*
Test2::E2::ice_staticId() noexcept
{
    return "::Test2::E2";
}

const char*
Test2::E2::ice_id() const noexcept
{
    return ice_staticId();
}

void
Test2::E2::ice_throw() const
{
    throw *this;
}

void
Test2::E2::_writeImpl(Ice::OutputStream* ostr) const
{
    ostr->startSlice(ice_staticId(), -1, false);
    ostr->writeAll(this->l);
    ostr->endSlice();
    E1::_writeImpl(ostr);
}

void
Test2::E2::_readImpl(Ice::InputStream* istr)
{
    istr->startSlice();
    istr->readAll(this->l);
    istr->endSlice();
    E1::_readImpl(istr);
}

const char*
Test3::C1::ice_staticId() noexcept
{
    return "::Test3::C1";
}

const char*
Test3::C1::ice_id() const noexcept
{
    return ice_staticId();
}

void
Test3::C1::ice_printFields(std::ostream& os) const
{
    Ice::print(os << "i = ", this->i);
}

Ice::ValuePtr
Test3::C1::_iceCloneImpl() const
{
    return CloneEnabler<C1>::clone(*this);
}

void
Test3::C1::_iceWriteImpl(Ice::OutputStream* ostr) const
{
    ostr->startSlice(ice_staticId(), -1, true);
    ostr->writeAll(this->i);
    ostr->endSlice();
}

void
Test3::C1::_iceReadImpl(Ice::InputStream* istr)
{
    istr->startSlice();
    istr->readAll(this->i);
    istr->endSlice();
}

const char*
Test3::C2::ice_staticId() noexcept
{
    return "::Test3::C2";
}

const char*
Test3::C2::ice_id() const noexcept
{
    return ice_staticId();
}

void
Test3::C2::ice_printFields(std::ostream& os) const
{
    C1::ice_printFields(os);
    Ice::print(os << ", l = ", this->l);
}

Ice::ValuePtr
Test3::C2::_iceCloneImpl() const
{
    return CloneEnabler<C2>::clone(*this);
}

void
Test3::C2::_iceWriteImpl(Ice::OutputStream* ostr) const
{
    ostr->startSlice(ice_staticId(), -1, false);
    ostr->writeAll(this->l);
    ostr->endSlice();
    C1::_iceWriteImpl(ostr);
}

void
Test3::C2::_iceReadImpl(Ice::InputStream* istr)
{
    istr->startSlice();
    istr->readAll(this->l);
    istr->endSlice();
    C1::_iceReadImpl(istr);
}

void
Test3::E1::ice_printFields(std::ostream& os) const
{
    Ice::print(os << "i = ", this->i);
}

const char*
Test3::E1::ice_staticId() noexcept
{
    return "::Test3::E1";
}

const char*
Test3::E1::ice_id() const noexcept
{
    return ice_staticId();
}

void
Test3::E1::ice_throw() const
{
    throw *this;
}

void
Test3::E1::_writeImpl(Ice::OutputStream* ostr) const
{
    ostr->startSlice(ice_staticId(), -1, true);
    ostr->writeAll(this->i);
    ostr->endSlice();
}

void
Test3::E1::_readImpl(Ice::InputStream* istr)
{
    istr->startSlice();
    istr->readAll(this->i);
    istr->endSlice();
}

void
Test3::E2::ice_printFields(std::ostream& os) const
{
    E1::ice_printFields(os);
    Ice::print(os << ", l = ", this->l);
}

const char*
Test3::E2::ice_staticId() noexcept
{
    return "::Test3::E2";
}

const char*
Test3::E2::ice_id() const noexcept
{
    return ice_staticId();
}

void
Test3::E2::ice_throw() const
{
    throw *this;
}

void
Test3::E2::_writeImpl(Ice::OutputStream* ostr) const
{
    ostr->startSlice(ice_staticId(), -1, false);
    ostr->writeAll(this->l);
    ostr->endSlice();
    E1::_writeImpl(ostr);
}

void
Test3::E2::_readImpl(Ice::InputStream* istr)
{
    istr->startSlice();
    istr->readAll(this->l);
    istr->endSlice();
    E1::_readImpl(istr);
}
