...
namespace Test
{
    ...
    public:
        ...
        // NOLINTNEXTLINE(modernize-use-nodiscard)
        std::function<void()> throwTest1NotifyAsync(std::function<void()> response, std::function<void(std::exception_ptr)> exception = nullptr, std::function<void(bool)> sent = nullptr, const Ice::Context& context = Ice::noExplicitContext) const;
