...
std::function<void()>
Test::InitialPrx::throwTest1NotifyAsync(std::function<void()> response, std::function<void(std::exception_ptr)> exception, std::function<void(bool)> sent, const Ice::Context& context) const
{
    return IceInternal::makeLambdaOutgoing<void>(std::move(response), std::move(exception), std::move(sent), this, &Test::InitialPrx::_iceI_throwTest1Notify, context);
}

void
Test::InitialPrx::_iceI_throwTest1Notify(const std::shared_ptr<IceInternal::OutgoingAsyncT<void>>& outAsync, const Ice::Context& context) const
{
    static constexpr std::string_view operationName = "throwTest1Notify";
...
/// \cond INTERNAL
void
Test::Initial::_iceD_throwTest1Notify(
    Ice::IncomingRequest& request,
    std::function<void(Ice::OutgoingResponse)> sendResponse) // NOLINT(performance-unnecessary-value-param)
{
    _iceCheckMode(Ice::OperationMode::Normal, request.current().mode);
    request.inputStream().skipEmptyEncapsulation();
    this->throwTest1Notify(request.current());
    sendResponse(Ice::makeEmptyOutgoingResponse(request.current()));
}
/// \endcond
...
Test::Initial::dispatch(Ice::IncomingRequest& request, std::function<void(Ice::OutgoingResponse)> sendResponse)
{
    ...
    switch (r.first - allOperations.begin())
    {
        ...
        case 13:
        {
            _iceD_throwTest1Notify(request, std::move(sendResponse));
            break;
        }
        case 14:
        {
