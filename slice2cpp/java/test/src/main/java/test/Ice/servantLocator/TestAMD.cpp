...
/// \cond INTERNAL
bool
Test::TestIntf::_iceD_javaException(::IceInternal::Incoming& incoming)
{
    _iceCheckMode(::Ice::OperationMode::Normal, incoming.current().mode);
    incoming.readEmptyParams();
    auto incomingPtr = ::std::make_shared<::IceInternal::Incoming>(::std::move(incoming));
    try
    {
        this->javaExceptionAsync([incomingPtr] { incomingPtr->response(); }, [incomingPtr](std::exception_ptr ex) { incomingPtr->completed(ex); }, incomingPtr->current());
    }
    catch (...)
    {
        incomingPtr->failed(::std::current_exception());
...
Test::TestIntf::_iceDispatch(::IceInternal::Incoming& incoming)
{
    ...
    switch(r.first - allOperations)
    {
        ...
        case 8:
        {
            return _iceD_javaException(incoming);
        }
        case 9:
        {
