// Copyright (c) ZeroC, Inc.

// slice2cpp version 3.8.0-alpha.0
// <auto-generated>Generated from Slice file 'NoPackage.ice'.</auto-generated>

#define ICE_BUILDING_GENERATED_CODE

#include "NoPackage.h"
#include <Ice/AsyncResponseHandler.h>
#include <Ice/DefaultSliceLoader.h>
#include <Ice/OutgoingAsync.h>
#include <algorithm>
#include <array>

#if defined(_MSC_VER)
#   pragma warning(disable : 4458) // declaration of ... hides class member
#   pragma warning(disable : 4996) // ... was declared deprecated
#elif defined(__clang__)
#   pragma clang diagnostic ignored "-Wshadow"
#   pragma clang diagnostic ignored "-Wdeprecated-declarations"
#elif defined(__GNUC__)
#   pragma GCC diagnostic ignored "-Wshadow"
#   pragma GCC diagnostic ignored "-Wdeprecated-declarations"
#endif

#ifndef ICE_DISABLE_VERSION
#   if ICE_INT_VERSION  != 30850
#       error Ice version mismatch: an exact match is required for beta generated code
#   endif
#endif

namespace
{
    const IceInternal::ClassInit<::Test1::C1> iceC_Test1_C1_init;
    const IceInternal::ClassInit<::Test1::C2> iceC_Test1_C2_init;
    const IceInternal::ExceptionInit<::Test1::E1> iceC_Test1_E1_init;
    const IceInternal::ExceptionInit<::Test1::E2> iceC_Test1_E2_init;
    const IceInternal::ExceptionInit<::Test1::notify> iceC_Test1_notify_init;
}

const char*
Test1::C1::ice_staticId() noexcept
{
    return "::Test1::C1";
}

const char*
Test1::C1::ice_id() const noexcept
{
    return ice_staticId();
}

void
Test1::C1::ice_printFields(std::ostream& os) const
{
    Ice::print(os << "i = ", this->i);
}

Ice::ValuePtr
Test1::C1::_iceCloneImpl() const
{
    return CloneEnabler<C1>::clone(*this);
}

void
Test1::C1::_iceWriteImpl(Ice::OutputStream* ostr) const
{
    ostr->startSlice(ice_staticId(), -1, true);
    ostr->writeAll(this->i);
    ostr->endSlice();
}

void
Test1::C1::_iceReadImpl(Ice::InputStream* istr)
{
    istr->startSlice();
    istr->readAll(this->i);
    istr->endSlice();
}

const char*
Test1::C2::ice_staticId() noexcept
{
    return "::Test1::C2";
}

const char*
Test1::C2::ice_id() const noexcept
{
    return ice_staticId();
}

void
Test1::C2::ice_printFields(std::ostream& os) const
{
    C1::ice_printFields(os);
    Ice::print(os << ", l = ", this->l);
}

Ice::ValuePtr
Test1::C2::_iceCloneImpl() const
{
    return CloneEnabler<C2>::clone(*this);
}

void
Test1::C2::_iceWriteImpl(Ice::OutputStream* ostr) const
{
    ostr->startSlice(ice_staticId(), -1, false);
    ostr->writeAll(this->l);
    ostr->endSlice();
    C1::_iceWriteImpl(ostr);
}

void
Test1::C2::_iceReadImpl(Ice::InputStream* istr)
{
    istr->startSlice();
    istr->readAll(this->l);
    istr->endSlice();
    C1::_iceReadImpl(istr);
}

void
Test1::E1::ice_printFields(std::ostream& os) const
{
    Ice::print(os << "i = ", this->i);
}

const char*
Test1::E1::ice_staticId() noexcept
{
    return "::Test1::E1";
}

const char*
Test1::E1::ice_id() const noexcept
{
    return ice_staticId();
}

void
Test1::E1::ice_throw() const
{
    throw *this;
}

void
Test1::E1::_writeImpl(Ice::OutputStream* ostr) const
{
    ostr->startSlice(ice_staticId(), -1, true);
    ostr->writeAll(this->i);
    ostr->endSlice();
}

void
Test1::E1::_readImpl(Ice::InputStream* istr)
{
    istr->startSlice();
    istr->readAll(this->i);
    istr->endSlice();
}

void
Test1::E2::ice_printFields(std::ostream& os) const
{
    E1::ice_printFields(os);
    Ice::print(os << ", l = ", this->l);
}

const char*
Test1::E2::ice_staticId() noexcept
{
    return "::Test1::E2";
}

const char*
Test1::E2::ice_id() const noexcept
{
    return ice_staticId();
}

void
Test1::E2::ice_throw() const
{
    throw *this;
}

void
Test1::E2::_writeImpl(Ice::OutputStream* ostr) const
{
    ostr->startSlice(ice_staticId(), -1, false);
    ostr->writeAll(this->l);
    ostr->endSlice();
    E1::_writeImpl(ostr);
}

void
Test1::E2::_readImpl(Ice::InputStream* istr)
{
    istr->startSlice();
    istr->readAll(this->l);
    istr->endSlice();
    E1::_readImpl(istr);
}

void
Test1::notify::ice_printFields(std::ostream& os) const
{
    Ice::print(os << "i = ", this->i);
}

const char*
Test1::notify::ice_staticId() noexcept
{
    return "::Test1::notify";
}

const char*
Test1::notify::ice_id() const noexcept
{
    return ice_staticId();
}

void
Test1::notify::ice_throw() const
{
    throw *this;
}

void
Test1::notify::_writeImpl(Ice::OutputStream* ostr) const
{
    ostr->startSlice(ice_staticId(), -1, true);
    ostr->writeAll(this->i);
    ostr->endSlice();
}

void
Test1::notify::_readImpl(Ice::InputStream* istr)
{
    istr->startSlice();
    istr->readAll(this->i);
    istr->endSlice();
}
