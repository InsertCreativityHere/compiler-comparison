...
public extension Ice.DefaultSliceLoader {
    @objc static func resolveTypeId_and_for() -> AnyObject.Type {
        for.self
    }
}
...
open class for: Ice.Value {
    ...
    public init(foo: Swift.Int32, from: execPrx?) {
        self.foo = foo
        self.from = from
    }
...
public extension Ice.DefaultSliceLoader {
    @objc static func resolveTypeId_and_is() -> AnyObject.Type {
        is.self
    }
}
...
public extension Ice.DefaultSliceLoader {
    @objc static func resolveTypeId_and_not() -> AnyObject.Type {
        not.self
    }
}
...
public struct importHelper {
    ...
    /// Write a `import` sequence to the stream.
    ///
    ...
    /// Write an optional `import?` sequence to the stream.
    ///
    /// - Parameters:
...
public struct inHelper {
    ...
    /// Write a `in` dictionary to the stream.
    ///
    /// - Parameter ostr: The stream to write to.
    ...
    /// Write an optional `in?` dictionary to the stream.
    ///
    /// - Parameters:
    ///   - ostr: The stream to write to.
    ///   - tag: The numeric tag associated with the value.
