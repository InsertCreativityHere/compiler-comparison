...
public extension BackgroundControllerPrx {
    ...
    ///
    /// - returns: `PromiseKit.Promise<>` - The result of the operation
    func initializeSocketStatusAsync(_ iceP_status: Swift.Int32, context: Ice.Context? = nil, sentOn: Dispatch.DispatchQueue? = nil, sentFlags: Dispatch.DispatchWorkItemFlags? = nil, sent: ((Swift.Bool) -> Swift.Void)? = nil) -> PromiseKit.Promise<Swift.Void> {
        return _impl._invokeAsync(operation: "initializeSocketStatus",
                                  mode: .Normal,
                                  write: { ostr in
                                      ostr.write(iceP_status)
                                  },
...
public struct BackgroundControllerDisp: Ice.Disp {
    ...
    public func dispatch(request: Ice.Request, current: Ice.Current) throws -> PromiseKit.Promise<Ice.OutputStream>? {
        ...
        case "initializeSocketStatus":
            return try servant._iceD_initializeSocketStatus(incoming: request, current: current)
        case "pauseCall":
...
public extension BackgroundController {
    ...
    func _iceD_initializeSocketStatus(incoming inS: Ice.Incoming, current: Ice.Current) throws -> PromiseKit.Promise<Ice.OutputStream>? {
        let iceP_status: Swift.Int32 = try inS.read { istr in
            let iceP_status: Swift.Int32 = try istr.read()
            return iceP_status
        }

        try self.initializeSocketStatus(status: iceP_status, current: current)
