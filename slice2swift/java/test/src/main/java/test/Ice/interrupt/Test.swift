...
public struct TestIntfControllerDisp: Ice.Disp {
    ...
    public func dispatch(request: Ice.Request, current: Ice.Current) throws -> PromiseKit.Promise<Ice.OutputStream>? {
        ...
        case "interrupt":
            return try servant._iceD_interrupt(incoming: request, current: current)
        case "resumeAdapter":
...
public extension TestIntfController {
    ...
    func _iceD_interrupt(incoming inS: Ice.Incoming, current: Ice.Current) throws -> PromiseKit.Promise<Ice.OutputStream>? {
        try inS.readEmptyParams()

        try self.interrupt(current: current)
