...
public extension Ice.DefaultSliceLoader {
    @objc static func resolveTypeId_Test_OptionalWithCustom() -> AnyObject.Type {
        OptionalWithCustom.self
    }
}
...
open class OptionalWithCustom: Ice.Value {
    ...
    open override func _iceReadImpl(from istr: Ice.InputStream) throws {
        _ = try istr.startSlice()
        nonisolated(unsafe) let iceP_self = self
        iceP_self.l = try SmallStructListHelper.read(from: istr, tag: 1)
        try istr.endSlice()
    }
