...
/// The exception that is thrown by a `Locator` implementation when it cannot find an object adapter with the
/// provided adapter ID.
...
public extension LocatorFinderPrx {
    /// Gets a proxy to the associated `Locator`. The proxy might point to several replicas.
    ///
    /// - Parameter context: Optional request context.
    ///
