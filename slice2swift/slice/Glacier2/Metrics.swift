// Copyright (c) ZeroC, Inc.

// slice2swift version 3.8.0-alpha.0
// <auto-generated>Generated from Slice file 'Metrics.ice'.</auto-generated>

import Foundation
import Ice

public extension Ice.DefaultSliceLoader {
    @objc static func resolveTypeId_IceMX_SessionMetrics() -> AnyObject.Type {
        SessionMetrics.self
    }
}

/// Provides information about Glacier2 sessions.
open class SessionMetrics: Ice.Metrics {
    /// The number of client requests forwarded.
    public var forwardedClient: Swift.Int32 = 0
    /// The number of server requests forwarded.
    public var forwardedServer: Swift.Int32 = 0
    /// The size of the routing table.
    public var routingTableSize: Swift.Int32 = 0
    /// The number of client requests queued.
    public var queuedClient: Swift.Int32 = 0
    /// The number of server requests queued.
    public var queuedServer: Swift.Int32 = 0
    /// The number of client requests overridden.
    public var overriddenClient: Swift.Int32 = 0
    /// The number of server requests overridden.
    public var overriddenServer: Swift.Int32 = 0

    public required init() {
        super.init()
    }

    public init(id: Swift.String, total: Swift.Int64 = 0, current: Swift.Int32 = 0, totalLifetime: Swift.Int64 = 0, failures: Swift.Int32 = 0, forwardedClient: Swift.Int32 = 0, forwardedServer: Swift.Int32 = 0, routingTableSize: Swift.Int32 = 0, queuedClient: Swift.Int32 = 0, queuedServer: Swift.Int32 = 0, overriddenClient: Swift.Int32 = 0, overriddenServer: Swift.Int32 = 0) {
        self.forwardedClient = forwardedClient
        self.forwardedServer = forwardedServer
        self.routingTableSize = routingTableSize
        self.queuedClient = queuedClient
        self.queuedServer = queuedServer
        self.overriddenClient = overriddenClient
        self.overriddenServer = overriddenServer
        super.init(id: id, total: total, current: current, totalLifetime: totalLifetime, failures: failures)
    }

    /// - Returns: The Slice type ID of the interface supported by this object.
    open override class func ice_staticId() -> Swift.String { "::IceMX::SessionMetrics" }

    open override func _iceReadImpl(from istr: Ice.InputStream) throws {
        _ = try istr.startSlice()
        nonisolated(unsafe) let iceP_self = self
        iceP_self.forwardedClient = try istr.read()
        iceP_self.forwardedServer = try istr.read()
        iceP_self.routingTableSize = try istr.read()
        iceP_self.queuedClient = try istr.read()
        iceP_self.queuedServer = try istr.read()
        iceP_self.overriddenClient = try istr.read()
        iceP_self.overriddenServer = try istr.read()
        try istr.endSlice()
        try super._iceReadImpl(from: istr);
    }

    open override func _iceWriteImpl(to ostr: Ice.OutputStream) {
        ostr.startSlice(typeId: SessionMetrics.ice_staticId(), compactId: -1, last: false)
        ostr.write(self.forwardedClient)
        ostr.write(self.forwardedServer)
        ostr.write(self.routingTableSize)
        ostr.write(self.queuedClient)
        ostr.write(self.queuedServer)
        ostr.write(self.overriddenClient)
        ostr.write(self.overriddenServer)
        ostr.endSlice()
        super._iceWriteImpl(to: ostr);
    }
}
