...
public extension RouterPrx {
    ...
    /// Creates a session for the client (caller) with the router. If a `SessionManager` is configured,
    ...
    ///
    /// - Returns: A proxy for the newly created session, or null if no `SessionManager` is configured.
    ///
    ...
    /// - Parameter context: Optional request context.
    ///
    /// - Returns: A proxy for the newly created session, or null if no `SSLSessionManager` is configured.
    ///
    /// - Throws: CannotCreateSessionException Thrown when the session cannot be created. PermissionDeniedException Thrown when an authentication or authorization failure occurs.
