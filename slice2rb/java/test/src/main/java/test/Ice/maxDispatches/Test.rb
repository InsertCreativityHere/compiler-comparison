# Copyright (c) ZeroC, Inc.

# slice2rb version 3.8.0-alpha.0
# <auto-generated>Generated from Slice file 'Test.ice'.</auto-generated>

require 'Ice'

module ::Test
    if not defined?(::Test::T_TestIntfPrx)
        T_TestIntfPrx = Ice::__declareProxy('::Test::TestIntf')
    end

    if not defined?(::Test::TestIntfPrx)
        module TestIntfPrx_mixin
            def op(context=nil)
                TestIntfPrx_mixin::OP_op.invoke(self, [], context)
            end

            def resetMaxConcurrentDispatches(context=nil)
                TestIntfPrx_mixin::OP_resetMaxConcurrentDispatches.invoke(self, [], context)
            end

            def shutdown(context=nil)
                TestIntfPrx_mixin::OP_shutdown.invoke(self, [], context)
            end
        end

        class TestIntfPrx < Ice::ObjectPrx
            include TestIntfPrx_mixin
        end

        T_TestIntfPrx.defineProxy(TestIntfPrx, nil, [])

        TestIntfPrx_mixin::OP_op = Ice::__defineOperation('op', 'op', Ice::OperationMode::Normal, nil, [], [], nil, [])
        TestIntfPrx_mixin::OP_resetMaxConcurrentDispatches = Ice::__defineOperation('resetMaxConcurrentDispatches', 'resetMaxConcurrentDispatches', Ice::OperationMode::Normal, nil, [], [], [Ice::T_int, false, 0], [])
        TestIntfPrx_mixin::OP_shutdown = Ice::__defineOperation('shutdown', 'shutdown', Ice::OperationMode::Normal, nil, [], [], nil, [])
    end

    if not defined?(::Test::T_ResponderPrx)
        T_ResponderPrx = Ice::__declareProxy('::Test::Responder')
    end

    if not defined?(::Test::ResponderPrx)
        module ResponderPrx_mixin
            def start(context=nil)
                ResponderPrx_mixin::OP_start.invoke(self, [], context)
            end

            def stop(context=nil)
                ResponderPrx_mixin::OP_stop.invoke(self, [], context)
            end

            def pendingResponseCount(context=nil)
                ResponderPrx_mixin::OP_pendingResponseCount.invoke(self, [], context)
            end
        end

        class ResponderPrx < Ice::ObjectPrx
            include ResponderPrx_mixin
        end

        T_ResponderPrx.defineProxy(ResponderPrx, nil, [])

        ResponderPrx_mixin::OP_start = Ice::__defineOperation('start', 'start', Ice::OperationMode::Normal, nil, [], [], nil, [])
        ResponderPrx_mixin::OP_stop = Ice::__defineOperation('stop', 'stop', Ice::OperationMode::Normal, nil, [], [], nil, [])
        ResponderPrx_mixin::OP_pendingResponseCount = Ice::__defineOperation('pendingResponseCount', 'pendingResponseCount', Ice::OperationMode::Normal, nil, [], [], [Ice::T_int, false, 0], [])
    end
end
