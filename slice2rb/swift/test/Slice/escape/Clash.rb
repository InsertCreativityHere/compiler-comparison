...
module ::Clash
    ...
    if not defined?(::Clash::Cls)
        class Cls < Ice::Value
            ...
            def initialize(s=nil, context='', current=0, response=0, upCast='', typeId=0, del=0, cookie=Ice::Unset, ex='', result=0, istr='', ostr='', inS='', proxy='')
                @s = s
                @context = context
                @current = current
                @response = response
            ...
            attr_accessor :s, :context, :current, :response, :upCast, :typeId, :del, :cookie, :ex, :result, :istr, :ostr, :inS, :proxy
        end
